...
// ==========================================
@interface SpeculumEditorViewController : UIViewController <UIDocumentPickerDelegate>
@property (nonatomic, strong) UIView *canvas;
@property (nonatomic, strong) NSMutableArray<NSMutableDictionary *> *widgets;
@property (nonatomic, strong) NSMutableArray<UILabel *> *labels;
@property (nonatomic, strong) UILabel *selectedLabel;
@end

@implementation SpeculumEditorViewController


- (void)validateFonts {
    BOOL missingFound = NO;
    NSString *docsDir = [NSSearchPathForDirectoriesInDomains(NSDocumentDirectory, NSUserDomainMask, YES) firstObject];
    
    for (NSMutableDictionary *dict in self.widgets) {
        NSString *savedFont = dict[@"fontPath"];
        if (savedFont.length > 0) {
            // extract name only
            NSString *fileName = [savedFont lastPathComponent];
            NSString *fullPath = [docsDir stringByAppendingPathComponent:fileName];
            
            if (![[NSFileManager defaultManager] fileExistsAtPath:fullPath]) {
                missingFound = YES;
                dict[@"fontPath"] = @""; // resets if fonts' not found
            } else {
                dict[@"fontPath"] = fileName; // save font name only
            }
        }
    }
    
    if (missingFound) {
        UIAlertController *ac = [UIAlertController alertControllerWithTitle:@"Missing Font" 
                                                                    message:@"One or more fonts, were not found and were reset to the default system one." 
...
- (void)viewDidLoad {
    ...
    self.navigationItem.rightBarButtonItem = [[UIBarButtonItem alloc] initWithImage:infoIcon
                                                                             ...
                                                                             target:self 
                                                                             action:@selector(showPatternsHelp)];
    self.view.backgroundColor = [UIColor systemBackgroundColor];
    
    // Load existing widgets from defaults
    NSArray *saved = [[NSUserDefaults standardUserDefaults] arrayForKey:@"SpeculumLiteWidgets"];
    self.widgets = [NSMutableArray array];
    if (saved) {
        for (NSDictionary *d in saved) {
            [self.widgets addObject:[d mutableCopy]];
        }
    }
...
- (void)setupCanvas {
    ...
    double canvasH = canvasW * (screenH / screenW);
    
    // fallback if the canvas is still too tall
    if (canvasH > availableHeight) {
        canvasH = availableHeight;
        canvasW = canvasH * (screenW / screenH);
    }
    
    // horizontal centered
    double startX = (self.view.bounds.size.width - canvasW) / 2.0;
    
    self.canvas = [[UIView alloc] initWithFrame:CGRectMake(startX, 120, canvasW, canvasH)];
    self.canvas.backgroundColor = [UIColor colorWithWhite:0.1 alpha:1.0];
    self.canvas.layer.cornerRadius = 24;
    self.canvas.layer.masksToBounds = YES;
    self.canvas.layer.borderWidth = 2;
    self.canvas.layer.borderColor = [UIColor darkGrayColor].CGColor;
    [self.view addSubview:self.canvas];
    
    UITapGestureRecognizer *tap = [[UITapGestureRecognizer alloc] initWithTarget:self action:@selector(deselectAll)];
...
- (NSString *)previewTextForTemplate:(NSString *)template {
   ...
   // ---------------------------------------------------------
    ...
    // ---------------------------------------------------------
    // 3. DATE & TIME (Fallback for remaining tags)
    // ---------------------------------------------------------
    NSError *error = nil;
...
- (void)renderAllWidgets {
    ...
    for (NSMutableDictionary *dict in self.widgets) {
        ...
        [self applyDict:dict toLabel:label];
...
- (CGFloat)previewScreenRatio {
    ...
    CGFloat widthRatio = canvasSize.width / screenSize.width;
    CGFloat heightRatio = canvasSize.height / screenSize.height;
    return MIN(widthRatio, heightRatio);
}

- (void)applyDict:(NSMutableDictionary *)dict toLabel:(UILabel *)label {
    label.transform = CGAffineTransformIdentity;

    // Text
    NSString *template = dict[@"textTemplate"] ?: @"New Text";
    label.text = [self previewTextForTemplate:template];

    // Color
    label.textColor = colorFromHexString(dict[@"hexColor"] ?: @"#FFFFFF");

    CGFloat logicalBaseFontSize = 20.0;
    CGFloat previewFontSize = logicalBaseFontSize * [self previewScreenRatio];

    // Local font loading for the preview
    ...
    if (fontPath.length > 0 && [[NSFileManager defaultManager] fileExistsAtPath:fontPath]) {
        ...
        if (provider) {
            ...
            if (cgFont) {
                NSString *psName =
                    (__bridge_transfer NSString *)CGFontCopyPostScriptName(cgFont);

                if (psName.length > 0) {
                    resolvedFont = [UIFont fontWithName:psName
    ...
    label.font = resolvedFont ?: [UIFont boldSystemFontOfSize:previewFontSize];
    [label sizeToFit];
    CGFloat scale = [dict[@"scaleSize"] doubleValue];
    if (scale <= 0.0) scale = 1.0;
    label.transform = CGAffineTransformMakeScale(scale, scale);
    CGFloat px = [dict[@"posX"] doubleValue];
    CGFloat py = [dict[@"posY"] doubleValue];
    label.center = CGPointMake(
        self.canvas.bounds.size.width * px,
        self.canvas.bounds.size.height * py
    );
}

// gestures and actions
- (void)handleTap:(UITapGestureRecognizer *)rec {
    [self selectLabel:(UILabel *)rec.view];
}

- (void)handlePan:(UIPanGestureRecognizer *)rec {
    UILabel *label = (UILabel *)rec.view;
    NSUInteger idx = [self.labels indexOfObject:label];
    if (idx == NSNotFound) return;
    
    if (rec.state == UIGestureRecognizerStateBegan) [self selectLabel:label];
    
    CGPoint trans = [rec translationInView:self.canvas];
    label.center = CGPointMake(label.center.x + trans.x, label.center.y + trans.y);
    [rec setTranslation:CGPointZero inView:self.canvas];
    
    if (rec.state == UIGestureRecognizerStateEnded) {
        self.widgets[idx][@"posX"] = @(label.center.x / self.canvas.bounds.size.width);
        self.widgets[idx][@"posY"] = @(label.center.y / self.canvas.bounds.size.height);
        [self saveState];
    }
}
...
- (void)handlePinch:(UIPinchGestureRecognizer *)rec {
    ...
    double currentScale = [self.widgets[idx][@"scaleSize"] doubleValue] ?: 1.0;
    
    if (rec.state == UIGestureRecognizerStateChanged) {
        double newScale = currentScale * rec.scale;
        label.transform = CGAffineTransformMakeScale(newScale, newScale);
    } else if (rec.state == UIGestureRecognizerStateEnded) {
        self.widgets[idx][@"scaleSize"] = @(currentScale * rec.scale);
        [self saveState];
    }
}
...
- (void)deselectAll {
    for (UILabel *l in self.labels) l.layer.borderWidth = 0;
    self.selectedLabel = nil;
}

//toolbar
- (void)addWidget {
    NSMutableDictionary *newWidget = [@{
        @"textTemplate": @"Today is {EEEE}",
        @"hexColor": @"#FFFFFF",
        @"scaleSize": @2.0,
        @"posX": @0.5,
        @"posY": @0.5
    ...
    [self renderAllWidgets];
    [self selectLabel:self.labels.lastObject];
    [self saveState];
}
...
- (void)editSelectedText {
    ...
    UIAlertController *ac = [UIAlertController alertControllerWithTitle:@"Edit Format" message:@"Use {} for Apple Date patterns (e.g., {HH:mm} or {EEEE}). For the full list press the top right button." preferredStyle:UIAlertControllerStyleAlert];
    [ac addTextFieldWithConfigurationHandler:^(UITextField *tf) {
        tf.text = self.widgets[idx][@"textTemplate"];
    }];
    [ac addAction:[UIAlertAction actionWithTitle:@"Save" style:UIAlertActionStyleDefault handler:^(UIAlertAction * _Nonnull action) {
        self.widgets[idx][@"textTemplate"] = ac.textFields.firstObject.text ?: @"";
        [self applyDict:self.widgets[idx] toLabel:self.selectedLabel];
        [self saveState];
    }]];
...
- (void)showPatternsHelp {
    NSString *title = @"Available Format Tags";
    
    NSString *message = 
        @"SYSTEM & HARDWARE:\n"
        @"• {battery} : Level (e.g. 85)\n"
        @"• {battery_status} : Status (Charging/Full/Unplugged)\n"
        @"• {device_name} : Device name\n"
        @"• {storage_free} : Free storage (e.g. 12.5 GB)\n"
        @"• {uptime} : System uptime (e.g. 2d 4h 12m)\n"
        @"• {ram_free} : Free RAM (e.g. 512 MB)\n\n"
        
        @"WEATHER & MEDIA:\n"
        @"• {temp} : Temperature\n"
        @"• {weather} : Condition text\n"
        @"• {weather_icon} : Weather icon\n"
        @"• {song} : Currently playing track\n\n"
        
        @"DATE & TIME (NSDate Formats):\n"
        @"• {HH:mm} : 24h Time (14:30)\n"
        @"• {hh:mm a} : 12h Time (02:30 PM)\n"
        @"• {EEEE} : Full day name (Monday)\n"
        @"• {MMMM d} : Month and day (August 5)\n"
        @"• {yyyy} : Year (2026)";
...
- (void)editSelectedColor {
    ...
    UIAlertController *ac = [UIAlertController alertControllerWithTitle:@"Edit Color" message:@"Enter Hex Code (e.g. #FF0000)" preferredStyle:UIAlertControllerStyleAlert];
    [ac addTextFieldWithConfigurationHandler:^(UITextField *tf) {
        tf.text = self.widgets[idx][@"hexColor"];
    }];
    [ac addAction:[UIAlertAction actionWithTitle:@"Save" style:UIAlertActionStyleDefault handler:^(UIAlertAction * _Nonnull action) {
        self.widgets[idx][@"hexColor"] = ac.textFields.firstObject.text ?: @"#FFFFFF";
        [self applyDict:self.widgets[idx] toLabel:self.selectedLabel];
        [self saveState];
    }]];
    [ac addAction:[UIAlertAction actionWithTitle:@"Cancel" style:UIAlertActionStyleCancel handler:nil]];
...
- (void)importSelectedFont {
    if (!self.selectedLabel) return;
    NSArray *types = @[@"public.font", @"public.truetype-ttf", @"public.opentype-font"];
...
- (void)documentPicker:(UIDocumentPickerViewController *)controller didPickDocumentsAtURLs:(NSArray<NSURL *> *)urls {
    ...
    if (!url || !self.selectedLabel) return;
    ...
    NSURL *destURL = [docsURL URLByAppendingPathComponent:url.lastPathComponent];
    ...
    if ([[NSFileManager defaultManager] copyItemAtURL:url toURL:destURL error:nil]) {
        NSUInteger idx = [self.labels indexOfObject:self.selectedLabel];
        self.widgets[idx][@"fontPath"] = destURL.lastPathComponent;
        [self applyDict:self.widgets[idx] toLabel:self.selectedLabel];
        [self saveState];
    }
...
// Live-sync to SpringBoard instantly whenever a drag or pinch stops
- (void)saveState {
    [[NSUserDefaults standardUserDefaults] setObject:self.widgets forKey:@"SpeculumLiteWidgets"];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    UITableViewCell *cell = [tableView dequeueReusableCellWithIdentifier:@"param-cell"];
    if (!cell) {
        cell = [[UITableViewCell alloc] initWithStyle:UITableViewCellStyleValue1 reuseIdentifier:@"param-cell"];
        cell.selectionStyle = UITableViewCellSelectionStyleNone;
    }
...
static NSString * const kSettingsPowercuffNominalNoticeShown = @"arsenic.powercuff.nominalDefaultNoticeShown.v1";
...
static double settings_drag_coefficient_value(NSUserDefaults *d)
{
    ...
    // Older Arsenic builds stored this row as an integer percent (50 = 0.50).
...
NSString * const kSettingsMagsafeStyle = @"MagsafeStyle";

NSString * const kSettingsNotweaficationsEnabled = @"NotweaficationsEnabled";

NSString * const kSettingsSpeculumLiteEnabled = @"SpeculumLiteEnabled";
NSString * const kSettingsSpeculumLiteHideDate = @"SpeculumLiteHideDate";
NSString * const kSettingsSpeculumLiteShowBattery = @"SpeculumLiteShowBattery";
NSString * const kSettingsSpeculumLiteCustomText = @"SpeculumLiteCustomText";
NSString * const kSettingsSpeculumLiteHexColor = @"SpeculumLiteHexColor";
NSString * const kSettingsSpeculumLiteFontPath = @"SpeculumLiteFontPath";
NSString * const kSettingsSpeculumLiteScaleSize = @"SpeculumLiteScaleSize";
...
static void arsenic_upload_log_if_enabled(void);
static void arsenic_upload_log_milestone(NSString *event);
static void arsenic_start_session_uploads(void);
static void arsenic_stop_session_uploads(void);
...
static volatile int g_notweafications_live_running = 0;
static volatile int g_notweafications_live_stop_requested = 0;
static volatile int g_speculumlite_live_running = 0;
static volatile int g_speculumlite_live_stop_requested = 0;
...
static void settings_request_notweafications_stop(void) { g_notweafications_live_stop_requested = 1; }
static void settings_request_speculumlite_stop(void) { g_speculumlite_live_stop_requested = 1; }
...
static BOOL settings_notweafications_running(void) { return g_notweafications_live_running != 0; }
static BOOL settings_speculumlite_running(void) { return g_speculumlite_live_running != 0; }
...
static bool settings_stop_magsafe_registered(BOOL springboardWillDie) {
    (void)springboardWillDie;
    return magsafe_tweak_stop_in_session();
}

static bool settings_stop_notweafications_registered(BOOL springboardWillDie)
{
    return notweafications_stop_in_session();
}

static bool settings_stop_speculumlite_registered(BOOL springboardWillDie) {
...
static const useconds_t kNotweaficationsLiveIntervalUS = 500000;
static const useconds_t kNotweaficationsLiveBackgroundIntervalUS = 1500000;
static const NSUInteger kNotweaficationsLiveMaxTicks = 43200;
static const useconds_t kSpeculumLiteLiveIntervalUS = 1000000;
static const useconds_t kSpeculumLiteLiveBackgroundIntervalUS = 2000000;
static const NSUInteger kSpeculumLiteLiveMaxTicks = 43200;
...
{
    return [@"ArsenicApplied." stringByAppendingString:key ?: @""];
}

static NSString *settings_persisted_applied_pid_key(NSString *key)
{
    return [@"ArsenicAppliedSpringBoardPID." stringByAppendingString:key ?: @""];
}
...
{
    return arsenic_experimental_tweaks_available() && settings_experimental_tweaks_enabled();
}
...
{
    return arsenic_experimental_tweaks_available();
}
...
static void settings_purge_legacy_access_auth(void)
{
    ...
    NSArray<NSString *> *keys = @[
        [@"Arsenic" stringByAppendingFormat:@"%@Linked", legacy],
        [@"Arsenic" stringByAppendingFormat:@"%@DisplayName", legacy],
        [@"Arsenic" stringByAppendingFormat:@"%@TierTitle", legacy],
        [@"Arsenic" stringByAppendingFormat:@"%@PledgeCents", legacy],
        [@"Arsenic" stringByAppendingFormat:@"%@LastRefresh", legacy],
    ];
    BOOL changed = NO;
...
//notweafications
static BOOL settings_notweafications_can_poll_springboard(void)
{
    (void)settings_refresh_screen_awake_state(NULL);
    return settings_screen_awake_cached();
}

static const char *settings_notweafications_pause_reason(void)
{
    if (!settings_screen_awake_cached()) return "screen asleep";
    return "screen unavailable";
}

static void settings_stop_notweafications_then_forget_locked(const char *reason)
{
    if (g_springboard_rc_ready) {
        bool stopped = notweafications_stop_in_session();
        printf("[SETTINGS] Notweafications stopped before state drop%s%s result=%d\n",
               reason ? ": " : "", reason ?: "", stopped);
    }
    notweafications_forget_remote_state();
}

//speculumlite
static BOOL settings_speculumlite_can_poll_springboard(void) {
    (void)settings_refresh_screen_awake_state(NULL);
    return settings_screen_awake_cached();
}

static const char *settings_speculumlite_pause_reason(void) {
    if (!settings_screen_awake_cached()) return "screen asleep";
    return "screen unavailable";
}

static void settings_stop_speculumlite_then_forget_locked(const char *reason) {
    if (g_springboard_rc_ready) {
        bool stopped = speculumlite_stop_in_session();
        printf("[SETTINGS] Speculum Lite stopped before state drop%s%s result=%d\n",
               reason ? ": " : "", reason ?: "", stopped);
    }
    speculumlite_forget_remote_state();
}
...
static void settings_begin_statbar_background_task_async(const char *reason)
{
    void (^beginTask)(void) = ^{
        @synchronized (settings_bg_lock()) {
            ...
            task = [app beginBackgroundTaskWithName:@"arsenic.statbar.live"
                                  expirationHandler:^{
                dispatch_async(dispatch_get_main_queue(), ^{
                    @synchronized (settings_bg_lock()) {
...
static NSString *settings_nicebar_weather_text_for_slot(NSUserDefaults *d, NSInteger slot)
{
    ...
    NSString *summary = ArsenicNiceBarWeatherSummary(codeNumber.integerValue, chinese);
                                                  ...
                                                  BOOL fetched)
{
    if ([temp isKindOfClass:NSNumber.class] && [code isKindOfClass:NSNumber.class]) {
        ...
        NSString *cache = [NSString stringWithFormat:@"%@ %.0f°",
                           ArsenicNiceBarWeatherSummary(code.integerValue, NO),
                           temp.doubleValue];
                                                       ...
                                                       void (^completion)(BOOL ok, NSString *text))
{
    ...
    dispatch_async(dispatch_get_main_queue(), ^{
        [[ArsenicNiceBarWeatherRefresher sharedRefresher]
    ...
    dispatch_async(dispatch_get_global_queue(QOS_CLASS_USER_INITIATED, 0), ^{
        @try {
            ...
            if (ok) {
                ...
                arsenic_upload_log_milestone(@"gravity-lite-applied");
            } else {
                log_user("[WARN] Gravity Lite did not start cleanly.\n");
                arsenic_upload_log_milestone(@"gravity-lite-warning");
            }
...
static void settings_start_statbar_live_loop(void)
{
    ...
    dispatch_async(dispatch_get_global_queue(0, 0), ^{
        ...
        arsenic_upload_log_milestone(@"statbar-live-started");
        ...
        @try {
            while ([d boolForKey:kSettingsStatBarEnabled] &&
                ...
                if (tick == 0) {
                    ...
                    arsenic_upload_log_milestone(ok ? @"statbar-live-first-ok" : @"statbar-live-first-failed");
                }
                if (ok) {
                    failures = 0;
                } else {
            ...
            if (failures > 0)
                arsenic_upload_log_milestone(@"statbar-live-exited-failed");
            __sync_lock_release(&g_statbar_live_running);
        }
    });
}
...
static void settings_start_rssi_live_loop(void)
{
    ...
    dispatch_async(dispatch_get_global_queue(0, 0), ^{
        ...
        arsenic_upload_log_milestone(@"rssi-live-started");
        ...
        @try {
            while ([d boolForKey:kSettingsRSSIDisplayEnabled] &&
                ...
                if (tick == 0) {
                    ...
                    arsenic_upload_log_milestone(ok ? @"rssi-live-first-ok" : @"rssi-live-first-failed");
                }
                if (ok) {
                    failures = 0;
                } else {
            ...
            printf("[SETTINGS] RSSI live loop exited ticks=%lu enabled=%d failures=%lu stop=%d\n",
                   ...
                   g_rssi_live_stop_requested);
            if (failures > 0)
                arsenic_upload_log_milestone(@"rssi-live-exited-failed");
            __sync_lock_release(&g_rssi_live_running);
        }
    });
}
...
static bool settings_speculumlite_apply(void) {
    NSUserDefaults *d = [NSUserDefaults standardUserDefaults];
    SpeculumLiteConfig config;
    memset(&config, 0, sizeof(config));
    
    config.enabled = YES;
    config.hideDate = [d boolForKey:kSettingsSpeculumLiteHideDate];
    config.hideTime = [d boolForKey:@"SpeculumLiteHideTime"];
    
    NSArray *savedWidgets = [d arrayForKey:@"SpeculumLiteWidgets"];
    config.widgetCount = 0;
    
    if (savedWidgets && [savedWidgets isKindOfClass:[NSArray class]]) {
        NSString *docsDir = [NSSearchPathForDirectoriesInDomains(NSDocumentDirectory, NSUserDomainMask, YES) firstObject];
        
        for (NSDictionary *w in savedWidgets) {
            if (config.widgetCount >= 16) break; // Cap at kSpeculumMaxWidgets
            
            NSString *text = w[@"textTemplate"] ?: @"";
            NSString *color = w[@"hexColor"] ?: @"#FFFFFF";
            NSString *fontName = w[@"fontPath"] ?: @"";
            NSString *absoluteFontPath = @"";
            
            if (fontName.length > 0) {
                // path for springboard
                absoluteFontPath = [docsDir stringByAppendingPathComponent:fontName];
            }
            
            strncpy(config.widgets[config.widgetCount].textTemplate, text.UTF8String, 255);
            // absolute path
            strncpy(config.widgets[config.widgetCount].fontPath, absoluteFontPath.UTF8String, 511);
            strncpy(config.widgets[config.widgetCount].hexColor, color.UTF8String, 15);
            
            config.widgets[config.widgetCount].scaleSize = w[@"scaleSize"] ? [w[@"scaleSize"] doubleValue] : 1.0;
            config.widgets[config.widgetCount].posX = w[@"posX"] ? [w[@"posX"] doubleValue] : 0.5;
            config.widgets[config.widgetCount].posY = w[@"posY"] ? [w[@"posY"] doubleValue] : 0.2;
            
            config.widgetCount++;
        }
    }
    
    return speculumlite_apply_in_session(config);
}
...
static void settings_start_axonlite_live_loop(void)
{
    ...
    dispatch_async(dispatch_get_global_queue(0, 0), ^{
        ...
        arsenic_upload_log_milestone(@"axon-lite-live-started");
        ...
        @try {
            ...
            while ([d boolForKey:kSettingsAxonLiteEnabled] &&
                ...
                if (tick == 0) {
                    ...
                    arsenic_upload_log_milestone(ok ? @"axon-lite-live-first-ok" : @"axon-lite-live-first-failed");
                }
                if (ok) {
                    failures = 0;
                } else {
            ...
            printf("[SETTINGS] Axon Lite live loop exited ticks=%lu enabled=%d failures=%lu stop=%d\n",
                   ...
                   g_axonlite_live_stop_requested);
            if (failures > 0)
                arsenic_upload_log_milestone(@"axon-lite-live-exited-failed");
            __sync_lock_release(&g_axonlite_live_running);
        }
    });
}
...
static void settings_start_notweafications_live_loop(void)
{
    if (!settings_device_supported()) return;
    if (settings_cleanup_in_progress()) return;

    NSUserDefaults *d = [NSUserDefaults standardUserDefaults];
    if (![d boolForKey:kSettingsNotweaficationsEnabled]) return;
    if (!g_springboard_rc_ready) return;

    if (__sync_lock_test_and_set(&g_notweafications_live_running, 1)) {
        static volatile int loggedAlready = 0;
        if (__sync_bool_compare_and_swap(&loggedAlready, 0, 1)) {
            printf("[SETTINGS] Notweafications live loop already running\n");
        }
        return;
    }

    if (settings_cleanup_in_progress()) {
        __sync_lock_release(&g_notweafications_live_running);
        return;
    }

    g_notweafications_live_stop_requested = 0;
    dispatch_async(dispatch_get_global_queue(0, 0), ^{
        NSUInteger tick = 0;
        NSUInteger failures = 0;
        uint64_t nextTickUS = settings_now_us();
        ...
        printf("[SETTINGS] Notweafications live loop started interval=%uus background=%uus max=%lu\n",
               kNotweaficationsLiveIntervalUS,
               kNotweaficationsLiveBackgroundIntervalUS,
               (unsigned long)kNotweaficationsLiveMaxTicks);
        arsenic_upload_log_milestone(@"notweafications-live-started");

        @try {
            settings_live_loop_sleep_interruptible(0,
                                                   settings_live_interval(kNotweaficationsLiveIntervalUS,
                                                                          kNotweaficationsLiveBackgroundIntervalUS),
                                                   &g_notweafications_live_stop_requested);
            nextTickUS = settings_now_us();
            while ([d boolForKey:kSettingsNotweaficationsEnabled] &&
                   !settings_cleanup_in_progress() &&
                   !g_notweafications_live_stop_requested &&
                   tick < kNotweaficationsLiveMaxTicks) {
                useconds_t intervalUS = settings_live_interval(kNotweaficationsLiveIntervalUS,
                                                               kNotweaficationsLiveBackgroundIntervalUS);
                
                if (!settings_notweafications_can_poll_springboard()) {
                    if (!pausedForUnavailableScreen) {
                        pausedForUnavailableScreen = YES;
                        printf("[SETTINGS] Notweafications paused while %s\n",
                               settings_notweafications_pause_reason());
                    }
                    settings_live_loop_sleep_interruptible(0,
                                                           intervalUS,
                                                           &g_notweafications_live_stop_requested);
                ...
                if (pausedForUnavailableScreen) {
                    pausedForUnavailableScreen = NO;
                    printf("[SETTINGS] Notweafications resumed after screen unlock/wake\n");
                }
                ...
                @synchronized (settings_rc_lock()) {
                    if (g_notweafications_live_stop_requested) break;
                    if (!g_springboard_rc_ready) {
                        printf("[SETTINGS] Notweafications loop has no SpringBoard RemoteCall session\n");
                        failures++;
                        break;
                    }
                    if (!settings_notweafications_can_poll_springboard()) {
                        printf("[SETTINGS] Notweafications tick skipped inside lock: %s\n",
                               settings_notweafications_pause_reason());
                        nextTickUS = settings_now_us();
                        continue;
                    }
                    ok = notweafications_apply_in_session();
                }

                if (tick == 0) {
                    printf("[SETTINGS] Notweafications result=%d\n", ok);
                    arsenic_upload_log_milestone(ok ? @"notweafications-live-first-ok" : @"notweafications-live-first-failed");
                }
                if (ok) {
                    failures = 0;
                } else {
                    failures++;
                    printf("[SETTINGS] Notweafications tick failed tick=%lu failures=%lu\n",
                           (unsigned long)tick, (unsigned long)failures);
                    if (failures >= settings_live_failure_limit(3)) break;
                }

                tick++;
                if (![d boolForKey:kSettingsNotweaficationsEnabled] ||
                    g_notweafications_live_stop_requested ||
                    tick >= kNotweaficationsLiveMaxTicks) break;

                uint64_t nowUS = settings_now_us();
                if (nextTickUS != 0) {
                    intervalUS = settings_live_interval(kNotweaficationsLiveIntervalUS,
                                                        kNotweaficationsLiveBackgroundIntervalUS);
                    nextTickUS += intervalUS;
                    if (nowUS < nextTickUS) {
                        settings_live_loop_sleep_interruptible(nextTickUS,
                ...
                uint64_t elapsedUS = tickStartUS != 0 && nowUS >= tickStartUS ? nowUS - tickStartUS : 0;
                if (tick == 1) {
                    printf("[SETTINGS] Notweafications tick=0 elapsed=%lluus\n", elapsedUS);
                }
            }
        } @finally {
            printf("[SETTINGS] Notweafications live loop exited ticks=%lu enabled=%d failures=%lu stop=%d\n",
                   (unsigned long)tick,
                   [d boolForKey:kSettingsNotweaficationsEnabled],
                   (unsigned long)failures,
                   g_notweafications_live_stop_requested);
            if (failures > 0)
                arsenic_upload_log_milestone(@"notweafications-live-exited-failed");
            __sync_lock_release(&g_notweafications_live_running);
        }
    });
}
...
// tracker for weather
static uint64_t g_last_speculum_weather_fetch_ms = 0;

static void speculum_update_media_cache(void) {
    static void *mrHandle = NULL;
    static void (*MRMediaRemoteGetNowPlayingInfo)(dispatch_queue_t, void(^)(CFDictionaryRef)) = NULL;
    
    // 1. Dynamically load MediaRemote the first time this runs
    if (!mrHandle) {
        mrHandle = dlopen("/System/Library/PrivateFrameworks/MediaRemote.framework/MediaRemote", RTLD_NOW);
        if (mrHandle) {
            MRMediaRemoteGetNowPlayingInfo = dlsym(mrHandle, "MRMediaRemoteGetNowPlayingInfo");
        }
    }
    
    // 2. Fetch Now Playing Info
    if (MRMediaRemoteGetNowPlayingInfo) {
        MRMediaRemoteGetNowPlayingInfo(dispatch_get_global_queue(0, 0), ^(CFDictionaryRef information) {
            ...
            NSUserDefaults *d = [NSUserDefaults standardUserDefaults];
            if (title.length > 0) {
                NSString *songStr = artist.length > 0 ? [NSString stringWithFormat:@"%@ - %@", artist, title] : title;
                [d setObject:songStr forKey:@"SpeculumLiteMediaSong"];
            } else {
                [d setObject:@"Not Playing" forKey:@"SpeculumLiteMediaSong"];
            }
        });
    }
}

static void speculum_update_weather_cache(void) {
    uint64_t now_ms = settings_now_ms();
    
    // only fetches weather every 15 minutes
    if (g_last_speculum_weather_fetch_ms != 0 && (now_ms - g_last_speculum_weather_fetch_ms) < 900000) {
        return;
    }
    g_last_speculum_weather_fetch_ms = now_ms;
    
    NSURL *url = [NSURL URLWithString:@"https://api.open-meteo.com/v1/forecast?latitude=41.89&longitude=12.49&current_weather=true"];
    ...
    [[[NSURLSession sharedSession] dataTaskWithURL:url completionHandler:^(NSData *data, NSURLResponse *response, NSError *error) {
        if (data && !error) {
            ...
            if (current) {
                NSNumber *temp = current[@"temperature"];
                NSNumber *weatherCode = current[@"weathercode"];
                
                // 1. Text Condition Mapping
                NSString *condition = (weatherCode.intValue < 3) ? @"Sunny" : @"Cloudy/Rain"; 
                
                // 2. emoji mapping (Open-Meteo Codes)
                NSString *icon = @"☁️"; // Default fallback
                int code = weatherCode.intValue;
                if (code == 0) icon = @"☀️";
                ...
                NSUserDefaults *d = [NSUserDefaults standardUserDefaults];
                [d setObject:[NSString stringWithFormat:@"%@°", temp] forKey:@"SpeculumLiteWeatherTemp"];
                [d setObject:condition forKey:@"SpeculumLiteWeatherCond"];
                [d setObject:icon forKey:@"SpeculumLiteWeatherIcon"];
            }
        } else {
            g_last_speculum_weather_fetch_ms = 0; 
        }
    }] resume];
}

static void settings_start_speculumlite_live_loop(void)
{
    if (!settings_device_supported()) return;
    if (settings_cleanup_in_progress()) return;

    NSUserDefaults *d = [NSUserDefaults standardUserDefaults];
    if (![d boolForKey:kSettingsSpeculumLiteEnabled]) return;
    if (!g_springboard_rc_ready) return;

    g_speculumlite_live_stop_requested = 0;

    if (__sync_lock_test_and_set(&g_speculumlite_live_running, 1)) {
        static volatile int loggedAlready = 0;
        if (__sync_bool_compare_and_swap(&loggedAlready, 0, 1)) {
            printf("[SETTINGS] Speculum Lite live loop already running\n");
        }
        return;
    }

    if (settings_cleanup_in_progress()) {
        __sync_lock_release(&g_speculumlite_live_running);
        return;
    }
    ...
    dispatch_async(dispatch_get_global_queue(0, 0), ^{
        ...
        printf("[SETTINGS] Speculum Lite live loop started interval=%uus background=%uus max=%lu\n",
               kSpeculumLiteLiveIntervalUS,
               kSpeculumLiteLiveBackgroundIntervalUS,
               (unsigned long)kSpeculumLiteLiveMaxTicks);

        @try {
            settings_live_loop_sleep_interruptible(0,
                                                   settings_live_interval(kSpeculumLiteLiveIntervalUS,
                                                                          kSpeculumLiteLiveBackgroundIntervalUS),
                                                   &g_speculumlite_live_stop_requested);
            nextTickUS = settings_now_us();
            while ([d boolForKey:kSettingsSpeculumLiteEnabled] &&
                   !settings_cleanup_in_progress() &&
                   !g_speculumlite_live_stop_requested &&
                   tick < kSpeculumLiteLiveMaxTicks) {
                useconds_t intervalUS = settings_live_interval(kSpeculumLiteLiveIntervalUS,
                                                               kSpeculumLiteLiveBackgroundIntervalUS);

                if (!settings_speculumlite_can_poll_springboard()) {
                    if (!pausedForUnavailableScreen) {
                        pausedForUnavailableScreen = YES;
                        printf("[SETTINGS] Speculum Lite paused while %s\n",
                               settings_speculumlite_pause_reason());
                    }
                    settings_live_loop_sleep_interruptible(0,
                                                           intervalUS,
                                                           &g_speculumlite_live_stop_requested);
                ...
                speculum_update_weather_cache();

                @synchronized (settings_rc_lock()) {
                    if (g_speculumlite_live_stop_requested) break;
                    if (!g_springboard_rc_ready) {
                        printf("[SETTINGS] Speculum Lite loop has no SpringBoard RemoteCall session\n");
                        break; 
                    }
                    if (!settings_speculumlite_can_poll_springboard()) {
                        nextTickUS = settings_now_us();
                ...
                tick++;
                if (![d boolForKey:kSettingsSpeculumLiteEnabled] ||
                    g_speculumlite_live_stop_requested ||
                    tick >= kSpeculumLiteLiveMaxTicks) break;

                uint64_t nowUS = settings_now_us();
                if (nextTickUS != 0) {
                    intervalUS = settings_live_interval(kSpeculumLiteLiveIntervalUS,
                                                        kSpeculumLiteLiveBackgroundIntervalUS);
                    nextTickUS += intervalUS;
                    if (nowUS < nextTickUS) {
                        settings_live_loop_sleep_interruptible(nextTickUS,
...
static void settings_start_notificationisland_live_loop(void)
{
    ...
    dispatch_async(dispatch_get_global_queue(0, 0), ^{
        ...
        arsenic_upload_log_milestone(@"notification-island-live-started");
        ...
        @try {
            while ([d boolForKey:kSettingsNotificationIslandEnabled] &&
                ...
                if (tick == 0) {
                    ...
                    arsenic_upload_log_milestone(ok ? @"notification-island-live-first-ok" :
            ...
            printf("[SETTINGS] Notification Island live loop exited ticks=%lu enabled=%d failures=%lu stop=%d\n",
                   ...
                   g_notificationisland_live_stop_requested);
            if (failures > 0)
                arsenic_upload_log_milestone(@"notification-island-live-exited-failed");
            __sync_lock_release(&g_notificationisland_live_running);
        }
    });
}
...
static void settings_apply_notweafications_once_async(const char *reason)
{
    if (!settings_device_supported()) return;
    if (settings_cleanup_in_progress()) return;
    if (g_notweafications_live_running) {
        if (reason) {
            printf("[SETTINGS] Notweafications lifecycle apply skipped: live loop owns Notweafications (%s)\n",
                   reason);
        }
        return;
    }
    ...
    NSUserDefaults *d = [NSUserDefaults standardUserDefaults];
    if (![d boolForKey:kSettingsNotweaficationsEnabled] || !g_springboard_rc_ready) return;

    dispatch_async(dispatch_get_global_queue(0, 0), ^{
        if (settings_cleanup_in_progress()) return;
        bool ok = false;
        if (!settings_notweafications_can_poll_springboard()) {
            printf("[SETTINGS] Notweafications lifecycle apply%s%s skipped: %s\n",
                   reason ? ": " : "", reason ?: "",
                   settings_notweafications_pause_reason());
            settings_start_notweafications_live_loop();
            return;
        }
        @synchronized (settings_rc_lock()) {
            if (settings_cleanup_in_progress() ||
                ...
                !g_springboard_rc_ready) return;
            if (!settings_notweafications_can_poll_springboard()) {
                printf("[SETTINGS] Notweafications lifecycle apply%s%s skipped inside lock: %s\n",
                       reason ? ": " : "", reason ?: "",
                       settings_notweafications_pause_reason());
                settings_start_notweafications_live_loop();
                return;
        ...
        printf("[SETTINGS] Notweafications lifecycle apply%s%s result=%d\n",
               reason ? ": " : "", reason ?: "", ok);
        settings_start_notweafications_live_loop();
    });
}
...
static void settings_apply_speculumlite_once_async(const char *reason)
{
    ...
    dispatch_async(dispatch_get_global_queue(0, 0), ^{
        ...
        bool ok = false;
        if (!settings_speculumlite_can_poll_springboard()) {
            settings_start_speculumlite_live_loop();
            return;
        }
        @synchronized (settings_rc_lock()) {
            if (settings_cleanup_in_progress() ||
                ![d boolForKey:kSettingsSpeculumLiteEnabled] ||
                !g_springboard_rc_ready) return;
            if (!settings_speculumlite_can_poll_springboard()) {
                settings_start_speculumlite_live_loop();
                return;
...
void settings_application_did_enter_background(void)
{
    ...
    BOOL anyLiveLoopNeeded =
        ...
        ([d boolForKey:kSettingsNotweaficationsEnabled]    && g_springboard_rc_ready) ||
    ...
    if ([d boolForKey:kSettingsNotweaficationsEnabled] && g_springboard_rc_ready) {
        settings_apply_notweafications_once_async("entered background");
    }
    if ([d boolForKey:kSettingsSpeculumLiteEnabled] && g_springboard_rc_ready) {
        settings_apply_speculumlite_once_async("entered background");
    }
...
void settings_application_will_enter_foreground(void)
{
    ...
    settings_apply_notweafications_once_async("will enter foreground");
    settings_apply_speculumlite_once_async("will enter foreground");
...
void settings_application_did_become_active(void)
{
    ...
    settings_apply_notweafications_once_async("became active");
    settings_apply_speculumlite_once_async("became active");
...
static BOOL settings_key_is_notweafications(NSString *key)
{
    return [key isEqualToString:kSettingsNotweaficationsEnabled];
}

static BOOL settings_key_is_speculumlite(NSString *key) {
    return [key isEqualToString:kSettingsSpeculumLiteEnabled] ||
           [key isEqualToString:kSettingsSpeculumLiteHideDate] ||
           [key isEqualToString:kSettingsSpeculumLiteShowBattery] ||
           [key isEqualToString:kSettingsSpeculumLiteCustomText] ||
           [key isEqualToString:kSettingsSpeculumLiteHexColor] ||
           [key isEqualToString:kSettingsSpeculumLiteFontPath] ||
           [key isEqualToString:kSettingsSpeculumLiteScaleSize] ||
           [key isEqualToString:kSettingsSpeculumLiteScaleHeight] ||
           [key isEqualToString:kSettingsSpeculumLiteOffsetX] ||
...
static void settings_schedule_live_apply_for_key(NSString *key)
{
    ...
    if ([key isEqualToString:kSettingsMagsafeEnabled]) {
            ...
            if (![d boolForKey:kSettingsMagsafeEnabled]) {
                settings_mark_tweak_applied(kSettingsMagsafeEnabled, NO);
                settings_notify_package_queue_changed_async();
                if (g_springboard_rc_ready) {
                    dispatch_async(dispatch_get_global_queue(0, 0), ^{
                        @synchronized (settings_rc_lock()) {
                            if (g_springboard_rc_ready) magsafe_tweak_stop_in_session();
                        }
                    });
                } else {
                    magsafe_tweak_forget_remote_state();
                }
            } else {
                settings_mark_tweak_needs_apply(kSettingsMagsafeEnabled);
                settings_notify_package_queue_changed_async();
            }
            return;
        }
    ...
    if (settings_key_is_notweafications(key)) {
        if ([d boolForKey:kSettingsNotweaficationsEnabled] && g_springboard_rc_ready) {
            dispatch_async(dispatch_get_global_queue(0, 0), ^{
                if (!settings_notweafications_can_poll_springboard()) {
                    printf("[SETTINGS] live Notweafications apply skipped: %s\n",
                           settings_notweafications_pause_reason());
                    settings_start_notweafications_live_loop();
                    settings_notify_package_queue_changed_async();
                    return;
                ...
                @synchronized (settings_rc_lock()) {
                    if (settings_cleanup_in_progress() || !g_springboard_rc_ready) return;
                    if (!settings_notweafications_can_poll_springboard()) {
                        printf("[SETTINGS] live Notweafications apply skipped inside lock: %s\n",
                               settings_notweafications_pause_reason());
                        settings_start_notweafications_live_loop();
                        settings_notify_package_queue_changed_async();
                        return;
                    }
                    bool ok = notweafications_apply_in_session();
                    settings_mark_tweak_applied(kSettingsNotweaficationsEnabled,
                                                ok && [d boolForKey:kSettingsNotweaficationsEnabled]);
                    printf("[SETTINGS] live Notweafications apply result=%d\n", ok);
                }
                settings_start_notweafications_live_loop();
                settings_notify_package_queue_changed_async();
            });
        } else if (![d boolForKey:kSettingsNotweaficationsEnabled]) {
            g_notweafications_live_stop_requested = 1;
            settings_mark_tweak_applied(kSettingsNotweaficationsEnabled, NO);
            settings_notify_package_queue_changed_async();
            if (g_springboard_rc_ready) {
                dispatch_async(dispatch_get_global_queue(0, 0), ^{
                    @synchronized (settings_rc_lock()) {
                        if (g_springboard_rc_ready) notweafications_stop_in_session();
                    }
                });
            }
        }
    ...
    if (settings_key_is_speculumlite(key)) {
        if ([d boolForKey:kSettingsSpeculumLiteEnabled] && g_springboard_rc_ready) {
            dispatch_async(dispatch_get_global_queue(0, 0), ^{
                if (!settings_speculumlite_can_poll_springboard()) {
                    settings_start_speculumlite_live_loop();
                ...
                @synchronized (settings_rc_lock()) {
                    if (settings_cleanup_in_progress() || ![d boolForKey:kSettingsSpeculumLiteEnabled] || !g_springboard_rc_ready) return;
                    if (!settings_speculumlite_can_poll_springboard()) {
                        settings_start_speculumlite_live_loop();
                        settings_notify_package_queue_changed_async();
                        return;
                    }
                    bool ok = settings_speculumlite_apply();
                    settings_mark_tweak_applied(kSettingsSpeculumLiteEnabled, ok && [d boolForKey:kSettingsSpeculumLiteEnabled]);
                }
                settings_start_speculumlite_live_loop();
                settings_notify_package_queue_changed_async();
            });
        } else if (![d boolForKey:kSettingsSpeculumLiteEnabled]) {
            g_speculumlite_live_stop_requested = 1;
            settings_mark_tweak_applied(kSettingsSpeculumLiteEnabled, NO);
            settings_notify_package_queue_changed_async();
            if (g_springboard_rc_ready) {
                dispatch_async(dispatch_get_global_queue(0, 0), ^{
                    @synchronized (settings_rc_lock()) {
                       if (g_springboard_rc_ready && ![d boolForKey:kSettingsSpeculumLiteEnabled]) {
                            speculumlite_stop_in_session();
                        }
                    }
                });
            }
...
void settings_register_defaults(void)
{
    ...
    [defaults registerDefaults:@{
        ...
        kSettingsNotweaficationsEnabled: @NO,
...
static void settings_run_actions_internal(BOOL pendingOnly)
{
    ...
    dispatch_async(dispatch_get_global_queue(0, 0), ^{
        ...
        @try {
            ...
            BOOL notweaficationsEnabled = [d boolForKey:kSettingsNotweaficationsEnabled];
            BOOL speculumLiteEnabled = [d boolForKey:kSettingsSpeculumLiteEnabled];
            ...
            arsenic_upload_log_milestone(@"run-plan");
            ...
            if (!hasRunWork) {
                ...
                if (!notweaficationsEnabled) g_notweafications_live_stop_requested = 1;
                if (!speculumLiteEnabled) g_speculumlite_live_stop_requested = 1;
                ...
                arsenic_upload_log_milestone(@"run-noop");
                return;
            }
            ...
            if (needsKernelPrimitiveStage) {
                ...
                if (!settings_ensure_kexploit()) {
                    ...
                    arsenic_upload_log_milestone(@"krw-failed");
                    return;
                }
                log_user("[OK] Kernel r/w armed — injection staged.\n");
                arsenic_upload_log_milestone(@"krw-ready");
                ...
                arsenic_upload_log_milestone(@"vphone-bridge-no-krw");
            }
            ...
            if (patchSandboxExt) {
                ...
                arsenic_upload_log_milestone(@"sandbox-ext-patched");
            }
            if (runPowercuff) {
                ...
                @synchronized (settings_rc_lock()) {
                    ...
                    arsenic_upload_log_milestone(ok ? @"powercuff-applied" : @"powercuff-failed");
                }
            }
            ...
            if (needsSpringBoard) {
                @synchronized (settings_rc_lock()) {
                    ...
                    if (!settings_ensure_springboard_remote_call_locked()) {
                        ...
                        arsenic_upload_log_milestone(@"springboard-remote-call-failed");
                        return;
                    }
                    log_user("[OK] SpringBoard channel open.\n");
                    arsenic_upload_log_milestone(@"springboard-remote-call-ready");
                    ...
                    if (runSandboxEscape && !g_springboard_sandbox_escaped) {
                        ...
                        arsenic_upload_log_milestone(sbx == 0 ? @"springboard-sandbox-token-ready" : @"springboard-sandbox-token-warning");
                        ...
                        arsenic_upload_log_milestone(@"springboard-sandbox-token-reused");
                    }
                    ...
                    if (cleanupDisabledSpringBoardTweaks) {
                        ...
                        arsenic_upload_log_milestone(@"disabled-springboard-tweaks-stopped");
                    }
                    ...
                    if (runTypeBanner) {
                        ...
                        arsenic_upload_log_milestone(ok ? @"typebanner-overlay-prewarmed" : @"typebanner-overlay-prewarm-failed");
                    }
                    ...
                    if (runSBC) {
                        ...
                        arsenic_upload_log_milestone(ok ? @"sbc-applied" : @"sbc-warning");
                    }
                    ...
                    if (runDarkTweaks) {
                        ...
                        log_user("%s DarkSword tweaks %s. Continuing with remaining changes…\n",
                                 ok ? "[OK]" : "[WARN]",
                                 ok ? "applied" : "may need a refresh");
                        arsenic_upload_log_milestone(ok ? @"darksword-tweaks-applied" : @"darksword-tweaks-warning");
                    }
                    ...
                    if (runLayoutExtras) {
                        ...
                        log_user("%s Home Layout Extras %s.\n",
                                 ok ? "[OK]" : "[WARN]",
                                 ok ? "applied" : "did not apply cleanly");
                        arsenic_upload_log_milestone(ok ? @"layout-extras-applied" : @"layout-extras-warning");
                    }
                    ...
                    if (runThemer) {
                        ...
                        log_user("%s Icon Theme Engine %s.\n",
                                 ok ? "[OK]" : "[WARN]",
                                 ok ? "applied" : "did not apply cleanly");
                        arsenic_upload_log_milestone(ok ? @"themer-applied" : @"themer-warning");
                        if (ok) {
                            settings_start_themer_live_loop();
                        }
                    }
                    ...
                    if (runSnowBoardLite) {
                        ...
                        arsenic_upload_log_milestone(ok ? @"snowboard-lite-applied" : @"snowboard-lite-warning");
                        if (ok && !settings_themer_live_repair_enabled(d)) {
                            log_user("[SBL] Live repair is enabled; Arsenic will keep the SpringBoard channel open so repair ticks reuse it.\n");
                            settings_start_themer_live_loop();
                        }
                    }
                    ...
                    if (runStatBar) {
                        ...
                        arsenic_upload_log_milestone(ok ? @"statbar-initial-applied" : @"statbar-initial-failed");
                    }
                    ...
                    if (runNSBar) {
                        ...
                        arsenic_upload_log_milestone(ok ? @"nsbar-initial-applied" : @"nsbar-initial-failed");
                    }
                    ...
                    if (runNiceBarLite) {
                        ...
                        arsenic_upload_log_milestone(ok ? @"nicebar-lite-initial-applied" : @"nicebar-lite-initial-failed");
                    }
                    ...
                    if (runRSSI) {
                        ...
                        arsenic_upload_log_milestone(ok ? @"rssi-initial-applied" : @"rssi-initial-failed");
                    }
                    ...
                    if (runLiveWP) {
                        ...
                        arsenic_upload_log_milestone(ok ? @"livewp-initial-applied" : @"livewp-initial-failed");
                    }
                    ...
                    if (runMagsafe) {
                        settings_progress(&step, total, "Applying Magsafe Enabler...");
                        
                        NSString *styleStr = [[NSUserDefaults standardUserDefaults] stringForKey:kSettingsMagsafeStyle];
                        int style = [styleStr isEqualToString:@"Native"] ? 0 : 1;
                        
                        bool ok = magsafe_tweak_init_in_session(style);
                        
                        UIDeviceBatteryState state = [UIDevice currentDevice].batteryState;
                        if (state == UIDeviceBatteryStateCharging || state == UIDeviceBatteryStateFull) {
                            float level = [UIDevice currentDevice].batteryLevel;
                            magsafe_tweak_show_animation(level, style);
                            dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(3.5 * NSEC_PER_SEC)), dispatch_get_global_queue(0, 0), ^{
                                        magsafe_tweak_hide_animation();
                                    });
                        } else {
                            magsafe_tweak_hide_animation();
                        }
                        
                        settings_mark_tweak_applied(kSettingsMagsafeEnabled, ok);
                        printf("[SETTINGS] Magsafe Enabler result=%d\n", ok);
                    }
                    ...
                    if (runAxonLite) {
                        ...
                        arsenic_upload_log_milestone(ok ? @"axon-lite-initial-applied" :
                                                     (deferred ? @"axon-lite-initial-deferred" : @"axon-lite-initial-failed"));
                    }

                    if (runNotweafications) {
                        settings_progress(&step, total, "Starting Notweafications coloring");
                        bool ok = false;
                        bool deferred = false;
                        if (settings_notweafications_can_poll_springboard()) {
                            ok = notweafications_apply_in_session();
                            ...
                            deferred = true;
                            printf("[SETTINGS] Notweafications initial apply skipped: %s\n",
                                   settings_notweafications_pause_reason());
                        }
                        settings_mark_tweak_applied(kSettingsNotweaficationsEnabled,
                                                    (ok || deferred) && [d boolForKey:kSettingsNotweaficationsEnabled]);
                        printf("[SETTINGS] Notweafications result=%d deferred=%d\n", ok, deferred);
                        log_user("%s Notweafications %s.\n",
                                 (ok || deferred) ? "[OK]" : "[WARN]",
                                 ok ? "active — styling notifications" :
                                 (deferred ? "standing by — fires when notifications appear" : "did not start cleanly"));
                        arsenic_upload_log_milestone(ok ? @"notweafications-initial-applied" :
                                                     (deferred ? @"notweafications-initial-deferred" : @"notweafications-initial-failed"));
                    }
                    
                    bool speculumLiteStartedNow = false;
                    if (runSpeculumLite) {
                        settings_progress(&step, total, "Starting Speculum Lite");

                        const bool enabled = [d boolForKey:kSettingsSpeculumLiteEnabled];

                        if (enabled && settings_speculumlite_can_poll_springboard()) {
                            speculumLiteStartedNow = settings_speculumlite_apply();
                        }

                        // IMPORTANT: "deferred" is not success. Leave the tweak pending until an
                        // apply call has actually located the lock-screen hierarchy and returned true.
                        settings_mark_tweak_applied(
                            kSettingsSpeculumLiteEnabled,
                            enabled && speculumLiteStartedNow
                        );

                        if (enabled && !speculumLiteStartedNow) {
                            settings_mark_tweak_needs_apply(kSettingsSpeculumLiteEnabled);
                        }
                    }
                    ...
                    if (runNotificationIsland) {
                        ...
                        arsenic_upload_log_milestone(ok ? @"notification-island-initial-applied" :
                    ...
                    if (runAppSwitcherGrid) {
                        ...
                        arsenic_upload_log_milestone(ok ? @"app-switcher-grid-applied" : @"app-switcher-grid-failed");
                    ...
                    if (runFastLockXLite) {
                        ...
                        arsenic_upload_log_milestone(ok ? @"fastlockx-lite-applied" :
                    ...
                    if (runStageStrip) {
                        ...
                        arsenic_upload_log_milestone(ok ? @"stagestrip-initial-applied" : @"stagestrip-initial-failed");
                ...
                if (runNotweafications) {
                    settings_start_notweafications_live_loop();
                } else if (!notweaficationsEnabled) {
                    g_notweafications_live_stop_requested = 1;
                }
                if (runSpeculumLite) {
                    g_speculumlite_live_stop_requested = 0;
                    settings_start_speculumlite_live_loop();
                } else if (!speculumLiteEnabled) {
                    g_speculumlite_live_stop_requested = 1;
                }
                if (runNotificationIsland) {
                    settings_start_notificationisland_live_loop();
            ...
            if (runTypeBanner) {
                ...
                arsenic_upload_log_milestone(@"typebanner-live-starting");
            ...
            if (runStatBar || runNSBar || runNiceBarLite || runRSSI || runAxonLite || runTypeBanner || runNotificationIsland || runLiveWP || runNotweafications || runSpeculumLite || startStageStripControlLoopAfterInstall)
                arsenic_upload_log_milestone(@"live-tweaks-started");
            ...
            if (runHadBlockingFailure) {
                ...
                arsenic_upload_log_milestone(@"run-incomplete");
                return;
            }
...
// Loads Arsenic/Changelog.plist (generated at build time by
...
@interface _ArsenicMailDelegate : NSObject <MFMailComposeViewControllerDelegate>
@end
@implementation _ArsenicMailDelegate
...
@end
static _ArsenicMailDelegate *_arsenic_mail_delegate(void) {
    static _ArsenicMailDelegate *d;
    static dispatch_once_t once;
    dispatch_once(&once, ^{ d = [[_ArsenicMailDelegate alloc] init]; });
    return d;
}
    ...
    if (indexPath.section == 0) {
        ...
        cell.detailTextLabel.text =
            @"Make a dictionary plist. Each key is a bundle ID. Each value is raw PNG data. "
             "Arsenic imports the plist and copies it into Documents/Themes.";
    } else {
        cell.selectionStyle = UITableViewCellSelectionStyleDefault;
        if (indexPath.row == 0) {
            ...
            cell.detailTextLabel.text = @"Exports Arsenic's bundled Info.plist for reference.";
        }
        cell.accessoryType = UITableViewCellAccessoryDisclosureIndicator;
    }
    return cell;
...
- (NSURL *)writeSamplePlist:(NSError **)error
{
    ...
    NSURL *url = [NSURL fileURLWithPath:
        [NSTemporaryDirectory() stringByAppendingPathComponent:@"ArsenicThemeTemplate.plist"]];
...
- (NSURL *)copyBuiltInIOS6Plist:(NSError **)error
{
    ...
    if (!src) {
        if (error) {
            *error = [NSError errorWithDomain:@"ArsenicThemerGuide"
                                         code:1
    ...
    NSURL *dst = [NSURL fileURLWithPath:
        [NSTemporaryDirectory() stringByAppendingPathComponent:@"Arsenic-iOS6-Theme.plist"]];
    NSFileManager *fm = NSFileManager.defaultManager;
...
- (NSURL *)copyAppInfoPlist:(NSError **)error
{
    ...
    NSURL *dst = [NSURL fileURLWithPath:
        [NSTemporaryDirectory() stringByAppendingPathComponent:@"Arsenic-Info.plist"]];
    NSFileManager *fm = NSFileManager.defaultManager;
    if ([fm fileExistsAtPath:dst.path]) {
        [fm removeItemAtURL:dst error:nil];
    }
...
- (NSArray<NSDictionary *> *)magsafeRows {
    return @[
        @{ @"kind": @"segmented", @"key": kSettingsMagsafeStyle, @"title": @"Design" }
    ];
}
...
- (NSArray<NSDictionary *> *)quickLoaderRows {
    ...
    //parsing
    if (self.qlRawScript && !self.qlParams) {
        if (!self.qlValues) {
...
{
    return @[
        @{ @"kind": @"toggle", @"key": kSettingsSpeculumLiteHideDate, @"title": @"Hide Native Date" },
        @{ @"kind": @"toggle", @"key": @"SpeculumLiteHideTime", @"title": @"Hide Native Time" },
        @{ @"kind": @"button", @"action": @"openSpeculumEditor", @"title": @"Open Layout Editor" }
    ];
}
...
- (NSArray<NSDictionary *> *)ipaDecryptorRows
{
    ...
    NSMutableArray<NSDictionary *> *rows = [NSMutableArray arrayWithArray:@[
        ...
        @{ @"kind": @"button",
           ...
           @"subtitle": @"Required before Arsenic can request an authenticated IPA download ticket. 2FA is requested after Apple asks for it.",
        ...
        @{ @"kind": @"info",
           ...
           @"subtitle": ipadecryptor_default_output_directory().length > 0
                ...
                : @"Arsenic Documents/DecryptedIPAs" },
        @{ @"kind": @"button",
...
- (NSArray<NSDictionary *> *)fastLockXLiteRows
{
    return @[
        ...
        @{ @"kind": @"button",
           ...
           @"subtitle": @"Keeps pickup-to-unlock armed after Arsenic closes.",
...
- (NSString *)tableView:(UITableView *)tableView titleForFooterInSection:(NSInteger)section
{
    ...
    if (s == SectionLaunch) {
        return @"kexploit_opa334 runs once per app lifetime. Keep Alive applies only while Arsenic is minimized; an App Switcher kill still terminates the process.";
    }
    if (s == SectionSBC) {
    ...
    if (s == SectionStatBar) {
        return @"Live overlay. When enabled, StatBar keeps a SpringBoard RemoteCall session open. Refresh rate applies when Arsenic is minimized but the screen is still awake; StatBar pauses while the screen is locked or asleep.";
    }
    if (s == SectionNSBar) {
    ...
    if (s == SectionAxonLite) {
        return @"RemoteCall-only Axon port. It uses a live app-side loop rather than substrate hooks, so it lasts for the active Arsenic SpringBoard session.";
    }
    if (s == SectionNotweafications) {
        return @"RemoteCall-only notification tweak. It colors notifications based on their Bundle ID for the active Arsenic SpringBoard session.";
    }
    ...
    if (s == SectionNotificationIsland) {
        return @"Experimental Dynamic Island notification route. Arsenic polls SpringBoard's active banner request through the shared RemoteCall session, then mirrors it through the app's ActivityKit Live Activity.";
    }
    ...
    if (s == SectionThemer) {
        return @"Legacy icon theme engine settings.\n\n"
               ...
               @"Custom themes can be a folder of PNG files named by bundle ID, such as com.apple.mobilesafari.png, or a binary plist mapping bundle IDs to PNG data. Import copies the theme into Arsenic's Documents/Themes folder. Theme Format Guide includes examples and plist exports.";
    }
    if (s == SectionSnowBoardLite) {
        return @"SnowBoard/IconBundles importer ported from d1y/cyanide-ios. Folder imports are copied into Arsenic's Documents/SnowBoardLite library and applied through the existing icon replacement pipeline.\n\nThe import copies theme assets into Arsenic's local storage so the original theme in Files is not changed.\n\nCompatibility: SnowBoard Lite keeps live icon repair active and reuses the SpringBoard RemoteCall channel between repair ticks. Re-run it after a respring if icons reset.";
    }
    if (s == SectionLiveWP) {
        return @"Video wallpaper ported from d1y/cyanide-ios. Select an MP4, MOV, or M4V; Arsenic copies it into Documents/LiveWP and plays it in SpringBoard while the RemoteCall session stays alive.";
    }
    return nil;
}
...
{
    NSURL *url = [NSURL URLWithString:@"https://github.com/MinePlayer16/arsenic/releases"];
...
- (UITableViewCell *)buildDocsCellInTableView:(UITableView *)tableView
{
    ...
    cell.detailTextLabel.textColor = UIColor.secondaryLabelColor;
    cell.detailTextLabel.text = @"How to write Arsenic tweaks";
    cell.accessoryType = UITableViewCellAccessoryDisclosureIndicator;
    cell.selectionStyle = UITableViewCellSelectionStyleDefault;
    return cell;
}
...
- (UITableViewCell *)buildAboutCellAtRow:(NSInteger)row tableView:(UITableView *)tableView
{
    ...
    switch (row) {
        case 0:
            ...
            cell.detailTextLabel.text = @"@mine_player16";
            break;
        case 1:
            cell.imageView.image = [SettingsViewController iconBadgeWithSymbol:@"at" color:UIColor.systemBlueColor size:29.0];
            cell.textLabel.text = @"Original Creator Twitter";
            cell.detailTextLabel.text = @"@0xjohnny";
            break;
        case 2:
            cell.imageView.image = [SettingsViewController iconBadgeWithSymbol:@"book.closed.fill" color:UIColor.systemPurpleColor size:29.0];
            cell.textLabel.text = @"Tweak SDK";
...
- (void)presentSnowBoardLiteArchiveImporter
{
    UIAlertController *hint = [UIAlertController
        alertControllerWithTitle:@"Import Theme Archive"
                         message:@"Pick a ZIP or DEB file that contains an IconBundles directory. Arsenic extracts and imports a local copy."
                  preferredStyle:UIAlertControllerStyleAlert];
    [hint addAction:[UIAlertAction actionWithTitle:@"Continue" style:UIAlertActionStyleDefault handler:^(UIAlertAction *a) {
        (void)a;
...
didPickDocumentsAtURLs:(NSArray<NSURL *> *)urls
{
    ...
    // =========================================================================
    // Speculum Lite (Custom Font Import)
    // =========================================================================
    if ([ext isEqualToString:@"ttf"] || [ext isEqualToString:@"otf"]) {
        ...
        BOOL copied = [[NSFileManager defaultManager] copyItemAtURL:url toURL:destURL error:&err];
        
        if (copied) {
            NSUserDefaults *d = [NSUserDefaults standardUserDefaults];
            [d setObject:destURL.path forKey:kSettingsSpeculumLiteFontPath];
            [d synchronize];
            ...
            UIAlertController *ac = [UIAlertController alertControllerWithTitle:@"Font Imported"
                                                                        message:[NSString stringWithFormat:@"\"%@\" is now ready for Speculum Lite.", url.lastPathComponent]
                                                                 preferredStyle:UIAlertControllerStyleAlert];
            [ac addAction:[UIAlertAction actionWithTitle:@"OK" style:UIAlertActionStyleDefault handler:nil]];
            [self presentViewController:ac animated:YES completion:nil];
            return;
        } else {
            // Fallthrough to the error log below if the copy fails
            log_user("[IMPORT] Failed to copy font file to Documents: %s\n", err.localizedDescription.UTF8String);
        }
    }
...
- (NSString *)compileQuickLoaderScript {
    ...
    NSMutableString *finalScript = [NSMutableString stringWithString:@"//Variables injected by Arsenic\n"];
    for (NSDictionary *param in self.qlParams) {
        NSString *varName = param[@"varName"];
...
- (void)applyQuickLoaderScript {
    ...
    NSLog(@"[Arsenic] Dynamic JS Tweak Saved Successfully!");
}
...
- (NSString *)nicebarSubtitleForSlot:(NSInteger)slot
{
    ...
    switch ((NiceBarLiteContentKind)kind) {
        ...
        case NiceBarLiteContentSystem: {
            ...
            if (item == NiceBarLiteSystemThermalState) {
                ...
                return [NSString stringWithFormat:@"%@ · %@",
                        ...
                        ArsenicNiceBarSystemLanguageName(language)];
            }
        ...
        case NiceBarLiteContentTimeFormat: {
            ...
            return ArsenicNiceBarTimeFormatName(format);
        }
...
- (void)presentNiceBarDateTimePickerForSlot:(NSInteger)slot
{
    ...
    __weak typeof(self) weakSelf = self;
    ArsenicNiceBarTimePresetPickerViewController *picker =
        [[ArsenicNiceBarTimePresetPickerViewController alloc] initWithSlotTitle:[NSString stringWithFormat:@"%@ Date / Time", settings_nicebar_slot_name(slot)]
...
- (void)presentNiceBarSystemPickerForSlot:(NSInteger)slot
{
    ...
    __weak typeof(self) weakSelf = self;
    ArsenicNiceBarSystemItemPickerViewController *picker =
        [[ArsenicNiceBarSystemItemPickerViewController alloc] initWithSlotTitle:[NSString stringWithFormat:@"%@ System Item", settings_nicebar_slot_name(slot)]
...
// pointing to Arsenic-Classic@{2,3}x.png at the bundle root. Modern is the
...
- (void)openTwitter
{
    NSURL *url = [NSURL URLWithString:@"https://twitter.com/mine_player16"];
    if (url) [[UIApplication sharedApplication] openURL:url options:@{} completionHandler:nil];
}

- (void)openTwitterOriginal
{
    NSURL *url = [NSURL URLWithString:@"https://twitter.com/0xjohnny"];
    if (url) [[UIApplication sharedApplication] openURL:url options:@{} completionHandler:nil];
}
...
- (void)openShareLog
{
    ...
    NSString *summary = [NSString stringWithFormat:@"Arsenic diagnostic log\nArsenic %@ · iOS %@ · %@",
...
#pragma mark - Telemetry Uploads (Disabled)

// Upload logs disabled
static void arsenic_upload_log_with_kind_event(NSString *kind, NSString *event) {}
static void arsenic_upload_log_with_kind(NSString *kind) {}
static void arsenic_upload_log_milestone(NSString *event) {}
static void arsenic_upload_log_if_enabled(void) {}
static void arsenic_start_session_uploads(void) {}
static void arsenic_stop_session_uploads(void) {}
...
void arsenic_present_contact(UIViewController *host)
{
    if (!host) return;
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if ([kind isEqualToString:@"number"]) {
     ...
     if ([kind isEqualToString:@"text"]) {
        ...
        objc_setAssociatedObject(field, "arsenicSettingsTextKey", key, OBJC_ASSOCIATION_RETAIN_NONATOMIC);
        objc_setAssociatedObject(field, "arsenicSettingsTextDefault", row[@"placeholder"] ?: @"",
    ...
    if ([kind isEqualToString:@"slider"]) {
        ...
        objc_setAssociatedObject(slider, "arsenicValueLabel", valueLabel, OBJC_ASSOCIATION_ASSIGN);
        objc_setAssociatedObject(slider, "arsenicUnit", unit, OBJC_ASSOCIATION_RETAIN);
        objc_setAssociatedObject(slider, "arsenicStep", @(step), OBJC_ASSOCIATION_RETAIN);
    ...
    if ([kind isEqualToString:@"segmented"]) {
        UITableViewCell *cell = [tableView dequeueReusableCellWithIdentifier:@"segmented" forIndexPath:dequeuePath];
                ...
                NSString *cur = [d stringForKey:row[@"key"]] ?: defaultValue;
                NSUInteger idx = [items indexOfObject:cur];
                if (idx == NSNotFound) idx = [items indexOfObject:defaultValue];
                ...
                if ([row[@"key"] isEqualToString:kSettingsMagsafeStyle]) {
                    UIAction *action = [UIAction actionWithHandler:^(__kindof UIAction * _Nonnull action) {
                        ...
                        [[NSUserDefaults standardUserDefaults] setObject:selected forKey:kSettingsMagsafeStyle];
                        [[NSUserDefaults standardUserDefaults] synchronize];
...
{
    if (!settings_device_supported()) return;
    NSNumber *stepNum = objc_getAssociatedObject(sender, "arsenicStep");
    NSInteger step = stepNum ? [stepNum integerValue] : 1;
    if (step <= 0) step = 1;
    NSInteger value = (NSInteger)llround((double)sender.value / (double)step) * step;
    UILabel *valueLabel = objc_getAssociatedObject(sender, "arsenicValueLabel");
    NSString *unit = objc_getAssociatedObject(sender, "arsenicUnit") ?: @"";
    if (valueLabel) {
        valueLabel.text = [NSString stringWithFormat:@"%ld%@", (long)value, unit];
    }
}
...
- (void)settingsTextFieldEditingEnded:(UITextField *)sender
{
    ...
    NSString *key = objc_getAssociatedObject(sender, "arsenicSettingsTextKey");
    if (key.length == 0) return;

    NSString *value = [sender.text stringByTrimmingCharactersInSet:
                       NSCharacterSet.whitespaceAndNewlineCharacterSet];
    NSString *defaultValue = objc_getAssociatedObject(sender, "arsenicSettingsTextDefault") ?: @"";
    ...
    if (apps.count == 0) {
        UIAlertController *ac = [UIAlertController
            alertControllerWithTitle:@"No Apps Found"
                             message:@"Arsenic could not list installed user apps yet. Run the chain once, then try again."
                      preferredStyle:UIAlertControllerStyleAlert];
        [ac addAction:[UIAlertAction actionWithTitle:@"OK" style:UIAlertActionStyleDefault handler:nil]];
        settings_present_controller(ac, self);
        return;
...
- (void)presentIPADecryptorSignInPrompt
{
    UIAlertController *ac = [UIAlertController
        alertControllerWithTitle:@"App Store Sign In"
                         message:@"Sign in with the Apple ID that owns or can download the app. If Apple asks for two-factor authentication, Arsenic will prompt for the code next."
                  preferredStyle:UIAlertControllerStyleAlert];
    [ac addTextFieldWithConfigurationHandler:^(UITextField *field) {
...
- (void)presentIPADecryptorAppStoreLinkPrompt
{
    ...
    UIAlertController *ac = [UIAlertController
        alertControllerWithTitle:@"App Store Link"
                         message:@"Paste an App Store URL like https://apps.apple.com/us/app/name/id123456789, or enter the numeric app ID. Arsenic will resolve it, then attempt the IPA download path."
                  preferredStyle:UIAlertControllerStyleAlert];
    [ac addTextFieldWithConfigurationHandler:^(UITextField *field) {
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
        ...
        switch ((RootSection)indexPath.section) {
            ...
            case RootSectionAbout: {
                switch (indexPath.row) {
                    ...
                    case 1: [self openTwitterOriginal]; break;
                    case 2: {
                    ...
                    case 3: [self showAppIconPicker]; break;
        ...
        if ([action isEqualToString:@"nano-load"]) {
            ...
            UIAlertController *ac = [UIAlertController
                alertControllerWithTitle:@"Seed Compatibility Index?"
                                 message:@"Adds this phone's product type to the local NanoRegistry compatibility-index MobileAsset and saves a .arsenic.bak backup beside the original file."
                          preferredStyle:UIAlertControllerStyleAlert];
            [ac addAction:[UIAlertAction actionWithTitle:@"Cancel" style:UIAlertActionStyleCancel handler:nil]];
    ...
    if (indexPath.section == SectionNiceBarLite) {
        ...
        if ([action isEqualToString:@"nicebar-traffic-history"]) {
            ArsenicNiceBarTrafficHistoryViewController *vc = [[ArsenicNiceBarTrafficHistoryViewController alloc] init];
            [self.navigationController pushViewController:vc animated:YES];
            return;
        }
    ...
    if (indexPath.section == SectionSpeculumLite) {
        NSDictionary *row = [self rowsForSection:indexPath.section][indexPath.row];
        if (![row[@"kind"] isEqualToString:@"button"]) return;
        
        NSString *action = row[@"action"];
        if ([action isEqualToString:@"openSpeculumEditor"]) {
            SpeculumEditorViewController *editor = [[SpeculumEditorViewController alloc] init];
...
- (void)importSpeculumFont
{
    self.pendingThemeImportMode = @"speculumlite";
    ...
    UIDocumentPickerViewController *dp = [[UIDocumentPickerViewController alloc] initWithDocumentTypes:documentTypes 
...
- (void)speculumSetText {
    UIAlertController *alert = [UIAlertController alertControllerWithTitle:@"Custom Text" 
                                                                   message:@"Leave empty to use the live clock." 
                                                            preferredStyle:UIAlertControllerStyleAlert];
    [alert addTextFieldWithConfigurationHandler:^(UITextField *textField) {
        textField.text = [[NSUserDefaults standardUserDefaults] stringForKey:kSettingsSpeculumLiteCustomText];
    }];
    [alert addAction:[UIAlertAction actionWithTitle:@"Save" style:UIAlertActionStyleDefault handler:^(UIAlertAction * _Nonnull action) {
        NSString *val = alert.textFields.firstObject.text ?: @"";
        [[NSUserDefaults standardUserDefaults] setObject:val forKey:kSettingsSpeculumLiteCustomText];
        [[NSUserDefaults standardUserDefaults] synchronize];
        settings_mark_tweak_needs_apply(kSettingsSpeculumLiteEnabled);
    }]];
    [alert addAction:[UIAlertAction actionWithTitle:@"Cancel" style:UIAlertActionStyleCancel handler:nil]];
...
- (void)speculumSetColor {
    UIAlertController *alert = [UIAlertController alertControllerWithTitle:@"Hex Color" 
                                                                   message:@"e.g. #FF0000 for Red" 
                                                            preferredStyle:UIAlertControllerStyleAlert];
    [alert addTextFieldWithConfigurationHandler:^(UITextField *textField) {
        textField.text = [[NSUserDefaults standardUserDefaults] stringForKey:kSettingsSpeculumLiteHexColor];
    }];
    [alert addAction:[UIAlertAction actionWithTitle:@"Save" style:UIAlertActionStyleDefault handler:^(UIAlertAction * _Nonnull action) {
        NSString *val = alert.textFields.firstObject.text ?: @"";
        [[NSUserDefaults standardUserDefaults] setObject:val forKey:kSettingsSpeculumLiteHexColor];
        [[NSUserDefaults standardUserDefaults] synchronize];
        settings_mark_tweak_needs_apply(kSettingsSpeculumLiteEnabled);
    }]];
    [alert addAction:[UIAlertAction actionWithTitle:@"Cancel" style:UIAlertActionStyleCancel handler:nil]];
