...
the Arsenic port author when known. Do not mention experimental source movement,
...
- [x] Fixed SnowBoard Lite imports on Signulous installs by copying selected themes into Arsenic instead of opening them in place.

### v1.2.19 - 2026-06-11

- [x] Added FastLockX Lite, available to Patrons for early access, as an experimental tool that automatically unlocks the screen once Face ID is accepted; tested on iOS 18 and iOS 26, with iOS 17 support not confirmed yet. Credits: original FastLockX author Artem Kasper and Arsenic port by zeroxjf.
...
- [x] Removed the standalone Arsenic Themer listing in favor of SnowBoard Lite.
...
- [x] Prevented Dynamic Stage Lite and Arsenic Themer live icon repair from running together to avoid SpringBoard resprings.

### v1.2.10 - 2026-06-05

- [x] Updated the Arsenic Signal group invite link.
...
- [x] Fixed LiveWP disappearing after leaving Arsenic and aligned NSBar/NiceBar Lite refresh loops with screen wake and sleep state.
...
- [x] Replaced the first-launch log collection notice with a Arsenic Signal group invite for feedback, feature requests, and support.
...
- [x] Allowed public Arsenic checkouts to build without optional experimental tweak sources.

### v1.2.4 - 2026-06-03

- [x] Added a StatBar refresh-rate setting and reduced battery use from repeated temperature polling.
- [x] Fixed Installer queue edge cases so no-op applies finish cleanly and pending activations are remembered after reopening Arsenic.
