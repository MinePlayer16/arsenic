...
static dispatch_queue_t quickloader_create_js_queue_locked(void) {
    NSString *label = [NSString stringWithFormat:@"com.iggy05.arsenic.quickloader.js.%llu",
...
bool quickloader_run_js_string(NSString *jsCode) {
    ...
    bool completed = quickloader_perform_sync_timeout(^{
        ...
        context[@"r_msg2_main_raw_double"] = ^() {
            if (!quickloader_generation_is_active(runGeneration)) return uint64_to_js(0);
            NSArray *args = [JSContext currentArguments];
            if (args.count < 3) return uint64_to_js(0);
            uint64_t target = js_to_uint64(args[0]);
            NSString *selector = [args[1] toString];
            
            double v1 = [args[2] toDouble];
            double v2 = args.count > 3 ? [args[3] toDouble] : 0.0;
            double v3 = args.count > 4 ? [args[4] toDouble] : 0.0;

            uint64_t res = r_msg2_main_raw(target, [selector UTF8String],
                                           &v1, sizeof(v1),
                                           args.count > 3 ? &v2 : NULL, args.count > 3 ? sizeof(v2) : 0,
                                           args.count > 4 ? &v3 : NULL, args.count > 4 ? sizeof(v3) : 0,
                                           NULL, 0);
            return uint64_to_js(res);
        };

        context[@"r_is_objc_ptr"] = ^NSNumber*(JSValue *val) {
            if (!quickloader_generation_is_active(runGeneration)) return @(0);
            return @(r_is_objc_ptr(js_to_uint64(val)));
        };

        context[@"r_responds_main"] = ^NSNumber*() {
            if (!quickloader_generation_is_active(runGeneration)) return @(0);
            NSArray *args = [JSContext currentArguments];
            if (args.count < 2) return @(0);
            uint64_t target = js_to_uint64(args[0]);
            NSString *selector = [args[1] toString];
            return @(r_responds_main(target, [selector UTF8String]));
        };

        context[@"r_ivar_value"] = ^NSString*() {
            if (!quickloader_generation_is_active(runGeneration)) return uint64_to_js(0);
            NSArray *args = [JSContext currentArguments];
            if (args.count < 2) return uint64_to_js(0);
            uint64_t target = js_to_uint64(args[0]);
            NSString *ivarName = [args[1] toString];
            uint64_t val = r_ivar_value(target, [ivarName UTF8String]);
            return uint64_to_js(val);
        };

        context[@"r_alloc_str"] = ^NSString*(NSString *str) {
            if (!quickloader_generation_is_active(runGeneration)) return uint64_to_js(0);
            if (!str) return uint64_to_js(0);
            uint64_t ptr = r_alloc_str([str UTF8String]);
            return uint64_to_js(ptr);
        };

        context[@"r_free"] = ^(JSValue *ptr) {
            if (!quickloader_generation_is_active(runGeneration)) return;
            r_free(js_to_uint64(ptr));
        };

        context[@"r_cfstr"] = ^NSString*(NSString *str) {
            if (!quickloader_generation_is_active(runGeneration)) return uint64_to_js(0);
            if (!str) return uint64_to_js(0);
            uint64_t ptr = r_cfstr([str UTF8String]);
            return uint64_to_js(ptr);
        };

        context[@"r_settle_us"] = ^NSNumber*(JSValue *us) {
            if (!quickloader_generation_is_active(runGeneration)) return @(0);
            return @(r_settle_us(js_to_uint64(us)));
        };

        context[@"r_dlsym_call"] = ^NSString*() {
            if (!quickloader_generation_is_active(runGeneration)) return uint64_to_js(0);
            NSArray *args = [JSContext currentArguments];
            if (args.count < 2) return uint64_to_js(0);
            
            uint32_t timeout = [args[0] toUInt32];
            NSString *symbol = [args[1] toString];
            
            uint64_t a1 = args.count > 2 ? js_to_uint64(args[2]) : 0;
            uint64_t a2 = args.count > 3 ? js_to_uint64(args[3]) : 0;
            uint64_t a3 = args.count > 4 ? js_to_uint64(args[4]) : 0;
            uint64_t a4 = args.count > 5 ? js_to_uint64(args[5]) : 0;
            uint64_t a5 = args.count > 6 ? js_to_uint64(args[6]) : 0;
            uint64_t a6 = args.count > 7 ? js_to_uint64(args[7]) : 0;
            uint64_t a7 = args.count > 8 ? js_to_uint64(args[8]) : 0;
            uint64_t a8 = args.count > 9 ? js_to_uint64(args[9]) : 0;
            
            uint64_t ret = r_dlsym_call(timeout, [symbol UTF8String], a1, a2, a3, a4, a5, a6, a7, a8);
            return uint64_to_js(ret);
        };

        context[@"r_msg2_main_struct_ret_rect"] = ^NSDictionary*() {
            if (!quickloader_generation_is_active(runGeneration)) return @{};
            NSArray *args = [JSContext currentArguments];
            ...
            uint64_t target = js_to_uint64(args[0]);
            NSString *selector = [args[1] toString];
            struct { double x, y, w, h; } rect = {0};
            
            r_msg2_main_struct_ret(target, [selector UTF8String], 
                                   &rect, sizeof(rect), 
        ...
        context[@"remote_read"] = ^NSArray*() {
            if (!quickloader_generation_is_active(runGeneration)) return @[];
            NSArray *args = [JSContext currentArguments];
            if (args.count < 2) return @[];
            
            uint64_t addr = js_to_uint64(args[0]);
            size_t size = [args[1] toUInt32];
            if (size == 0 || size > (1024 * 1024)) return @[]; // limited for safety to 1MB
            
            uint8_t *buf = malloc(size);
            if (!buf) return @[];
            remote_read(addr, buf, size);
        ...
        context[@"remote_write"] = ^(JSValue *addrVal, JSValue *bytesArr) {
            if (!quickloader_generation_is_active(runGeneration)) return;
            uint64_t addr = js_to_uint64(addrVal);
            if (![bytesArr isArray]) return;
            
            NSArray *arr = [bytesArr toArray];
            ...
            for (size_t i = 0; i < size; i++) {
                buf[i] = (uint8_t)[arr[i] unsignedIntValue];
            }
            remote_write(addr, buf, size);
            free(buf);
        };
