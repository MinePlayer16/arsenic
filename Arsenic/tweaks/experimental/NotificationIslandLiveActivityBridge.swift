...
final class NotificationIslandLiveActivityBridge: NSObject {
    ...
    @objc static func showSample() -> Bool {
        ...
        return show(
            ...
            body: "Ready.",
            source: "Arsenic",
            requestIdentifier: "arsenic.sample.\(Date().timeIntervalSince1970)"
        )
    }
...
private actor NotificationIslandLiveActivityController {
    ...
    func prepare() async -> Bool {
        ...
        do {
            let attributes = NotificationIslandActivityAttributes(stableIdentifier: "arsenic.notification-island")
    ...
    private func quietState() -> NotificationIslandActivityAttributes.ContentState {
        NotificationIslandActivityAttributes.ContentState(
            ...
            body: "",
            source: "",
            requestIdentifier: "arsenic.notification-island.quiet",
            sequence: sequence,
            isVisible: false
        )
    }
    ...
    private func alertTitle(title: String, source: String) -> String {
        guard !source.isEmpty, source != "Arsenic", source != title else {
    ...
    private func alertBody(title: String, body: String, source: String) -> String {
        let value: String
        if !source.isEmpty, source != "Arsenic", source != title {
