...
typedef NS_ENUM(NSInteger, ArsenicNiceBarTrafficRange) {
    ArsenicNiceBarTrafficRangeWeek = 0,
    ArsenicNiceBarTrafficRangeMonth = 1,
    ArsenicNiceBarTrafficRangeYear = 2,
};

@interface ArsenicNiceBarTrafficItem : NSObject
@property (nonatomic, copy) NSString *label;
@property (nonatomic, assign) uint64_t bytes;
@end

@implementation ArsenicNiceBarTrafficItem
@end

@interface ArsenicNiceBarTrafficChartView : UIView
@property (nonatomic, copy) NSArray<ArsenicNiceBarTrafficItem *> *items;
@end

@implementation ArsenicNiceBarTrafficChartView
...
- (void)setItems:(NSArray<ArsenicNiceBarTrafficItem *> *)items
{
    _items = [items copy];
...
- (void)drawRect:(CGRect)rect
{
    ...
    uint64_t maxBytes = 1;
    for (ArsenicNiceBarTrafficItem *item in self.items) {
    ...
    for (NSUInteger i = 0; i < count; i++) {
        ArsenicNiceBarTrafficItem *item = self.items[i];
...
@interface ArsenicNiceBarTrafficSummaryCell : UITableViewCell
@property (nonatomic, strong) UILabel *summaryLabel;
@property (nonatomic, strong) UILabel *detailLabelView;
@property (nonatomic, strong) ArsenicNiceBarTrafficChartView *chartView;
- (void)configureWithTitle:(NSString *)title
                     total:(uint64_t)total
                     items:(NSArray<ArsenicNiceBarTrafficItem *> *)items;
@end

@implementation ArsenicNiceBarTrafficSummaryCell
...
- (instancetype)initWithStyle:(UITableViewCellStyle)style reuseIdentifier:(NSString *)reuseIdentifier
{
    ...
    _chartView = [[ArsenicNiceBarTrafficChartView alloc] initWithFrame:CGRectZero];
...
@interface ArsenicNiceBarTrafficDetailCell : UITableViewCell
@end

@implementation ArsenicNiceBarTrafficDetailCell
...
@interface ArsenicNiceBarTrafficHistoryViewController ()
@property (nonatomic, assign) ArsenicNiceBarTrafficRange selectedRange;
@property (nonatomic, copy) NSDictionary<NSString *, NSString *> *history;
@property (nonatomic, copy) NSArray<ArsenicNiceBarTrafficItem *> *items;
@property (nonatomic, assign) uint64_t totalBytes;
@end

@implementation ArsenicNiceBarTrafficHistoryViewController
...
- (instancetype)init
{
    ...
    if (self) {
        ...
        _selectedRange = ArsenicNiceBarTrafficRangeWeek;
    }
    return self;
}
...
{
    [super viewDidLoad];
    [self.tableView registerClass:ArsenicNiceBarTrafficSummaryCell.class forCellReuseIdentifier:@"summary"];
    [self.tableView registerClass:ArsenicNiceBarTrafficDetailCell.class forCellReuseIdentifier:@"detail"];
...
- (ArsenicNiceBarTrafficItem *)itemWithLabel:(NSString *)label bytes:(uint64_t)bytes
{
    ArsenicNiceBarTrafficItem *item = [[ArsenicNiceBarTrafficItem alloc] init];
    item.label = label;
    item.bytes = bytes;
    return item;
}
...
- (void)rebuildItems
{
    NSMutableArray<ArsenicNiceBarTrafficItem *> *items = [NSMutableArray array];
    NSCalendar *calendar = NSCalendar.currentCalendar;
    NSDate *now = [NSDate date];
    NSDateFormatter *keyFormatter = [self dateFormatter];
    self.totalBytes = 0;

    if (self.selectedRange == ArsenicNiceBarTrafficRangeYear) {
        ...
        NSInteger daysBack = self.selectedRange == ArsenicNiceBarTrafficRangeWeek ? 6 : 29;
        NSDateFormatter *labelFormatter = [[NSDateFormatter alloc] init];
        labelFormatter.dateFormat = self.selectedRange == ArsenicNiceBarTrafficRangeWeek ? @"EEE" : @"M/d";
...
- (NSString *)rangeTitle
{
    switch (self.selectedRange) {
        case ArsenicNiceBarTrafficRangeWeek: return @"This Week";
        case ArsenicNiceBarTrafficRangeMonth: return @"This Month";
        case ArsenicNiceBarTrafficRangeYear: return @"This Year";
    }
    return @"Traffic";
}
...
{
    if (section == 2) return self.selectedRange == ArsenicNiceBarTrafficRangeYear ? @"Monthly Details" : @"Daily Details";
    return nil;
}
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (indexPath.section == 1) {
        ArsenicNiceBarTrafficSummaryCell *cell = [tableView dequeueReusableCellWithIdentifier:@"summary" forIndexPath:indexPath];
        [cell configureWithTitle:[self rangeTitle] total:self.totalBytes items:self.items];
        return cell;
    }

    ArsenicNiceBarTrafficDetailCell *cell = [tableView dequeueReusableCellWithIdentifier:@"detail" forIndexPath:indexPath];
    ArsenicNiceBarTrafficItem *item = self.items[(NSUInteger)indexPath.row];
...
{
    self.selectedRange = (ArsenicNiceBarTrafficRange)sender.selectedSegmentIndex;
    [self rebuildItems];
...
NSString *ArsenicNiceBarTimeFormatName(NSString *format)
{
    ...
    if ([format isEqualToString:@"arsenic:cn-date-weekday"]) return @"中文日期+星期";
    if ([format isEqualToString:@"M月d日 EEE"]) return @"中文日期+星期";
    if ([format isEqualToString:@"arsenic:lunar"]) return @"Lunar date";
    if ([format isEqualToString:@"arsenic:lunar-cn"]) return @"农历";
    if ([format isEqualToString:@"arsenic:lunar-cn-full"]) return @"农历完整";
    return format.length ? format : @"HH:mm";
}

static NSString *ArsenicNiceBarLunarCNPreview(BOOL full)
{
...
static BOOL ArsenicNiceBarTimeFormatUsesChineseLocale(NSString *format)
{
    return [format isEqualToString:@"a h:mm"] ||
           [format rangeOfString:@"月"].location != NSNotFound;
}

static NSString *ArsenicNiceBarChineseWeekdayPreview(void)
{
...
{
    if ([format isEqualToString:@"arsenic:lunar"]) {
        NSCalendar *cal = [NSCalendar calendarWithIdentifier:NSCalendarIdentifierChinese];
        NSDateComponents *c = [cal components:NSCalendarUnitMonth | NSCalendarUnitDay fromDate:[NSDate date]];
    ...
    if ([format isEqualToString:@"arsenic:lunar-cn"]) return ArsenicNiceBarLunarCNPreview(NO);
    if ([format isEqualToString:@"arsenic:lunar-cn-full"]) return ArsenicNiceBarLunarCNPreview(YES);
    if ([format isEqualToString:@"arsenic:cn-date-weekday"] ||
        [format isEqualToString:@"M月d日 EEE"]) {
        return [NSString stringWithFormat:@"%@ %@",
                ArsenicNiceBarPreviewForTimeFormat(@"M月d日"),
                ArsenicNiceBarChineseWeekdayPreview()];
    }
    NSDateFormatter *formatter = [[NSDateFormatter alloc] init];
    formatter.locale = ArsenicNiceBarTimeFormatUsesChineseLocale(format)
...
static NSArray<NSDictionary<NSString *, NSString *> *> *ArsenicNiceBarTimePresets(void)
{
    return @[
        ...
        @{ @"section": @"中文", @"title": @"中文日期+星期",    @"format": @"arsenic:cn-date-weekday" },
        @{ @"section": @"农历", @"title": @"Lunar date",     @"format": @"arsenic:lunar" },
        @{ @"section": @"农历", @"title": @"农历",           @"format": @"arsenic:lunar-cn" },
        @{ @"section": @"农历", @"title": @"农历完整",        @"format": @"arsenic:lunar-cn-full" },
    ];
}

@interface ArsenicNiceBarTimePresetPickerViewController ()
@property (nonatomic, copy) NSString *selectedFormat;
@property (nonatomic, copy) ArsenicNiceBarTimeFormatSelection selection;
...
@implementation ArsenicNiceBarTimePresetPickerViewController
                        ...
                        selection:(ArsenicNiceBarTimeFormatSelection)selection
{
    ...
    if (self) {
        ...
        for (NSDictionary<NSString *, NSString *> *preset in ArsenicNiceBarTimePresets()) {
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    cell.detailTextLabel.text = ArsenicNiceBarPreviewForTimeFormat(format);
...
@interface ArsenicNiceBarSystemItemPickerViewController ()
@property (nonatomic, assign) NSInteger selectedItem;
@property (nonatomic, copy) NSString *selectedLanguage;
@property (nonatomic, copy) ArsenicNiceBarSystemItemSelection selection;
@end

@implementation ArsenicNiceBarSystemItemPickerViewController
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (indexPath.section == 0) {
        ...
        cell.textLabel.text = ArsenicNiceBarSystemName(item);
        cell.detailTextLabel.text = ArsenicNiceBarSystemDescription(item);
        ...
        cell.textLabel.text = ArsenicNiceBarSystemLanguageName(language);
...
@interface ArsenicNiceBarWeatherRefresher () <CLLocationManagerDelegate>
@property (nonatomic, strong) CLLocationManager *locationManager;
@property (nonatomic, strong) NSMutableArray<ArsenicNiceBarWeatherCompletion> *pendingCompletions;
...
@implementation ArsenicNiceBarWeatherRefresher

+ (instancetype)sharedRefresher
{
    static ArsenicNiceBarWeatherRefresher *refresher;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        refresher = [[ArsenicNiceBarWeatherRefresher alloc] init];
    });
    return refresher;
}
...
- (void)fetchWeatherForLocation:(CLLocation *)location
{
    ...
    [[[NSURLSession sharedSession] dataTaskWithURL:url
        ...
        dispatch_async(dispatch_get_main_queue(), ^{
            ...
            NSString *summary = ArsenicNiceBarWeatherSummary(code.integerValue, NO);
             ...
             fetched:(BOOL)fetched
{
    ...
    NSArray<ArsenicNiceBarWeatherCompletion> *callbacks = [self.pendingCompletions copy];
    [self.pendingCompletions removeAllObjects];
    for (ArsenicNiceBarWeatherCompletion callback in callbacks) {
