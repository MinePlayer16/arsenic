...
PBXPROJ="Arsenic.xcodeproj/project.pbxproj"
...
#
#     Rules:
#       - Untracked Arsenic/tweaks/<name>.m       → "Add <Pretty Name> tweak"
...
compute_extra_bullets() {
    ...
    while IFS= read -r f; do
        ...
        add_bullet "Add ${pretty} tweak" "$pretty"
             ...
             | grep -E '^Arsenic/tweaks/.*\.m$' || true)

    while IFS= read -r name; do
        [ -z "$name" ] && continue
        add_bullet "Add ${name} package" "$name"
    done < <(git diff --no-color -- Arsenic/installer/PackageCatalog.m 2>/dev/null \
             | grep -E '^\+[[:space:]]+name:@"' \
             | sed -E 's/^\+[[:space:]]+name:@"([^"]+)".*/\1/' \
             | head -10)

    if printf '%s\n' "$changed_files" | grep -Eq '^Arsenic/installer/'; then
        add_bullet "Polish package installer queue, badges, and activity status UI" "installer"
    fi
    if printf '%s\n' "$changed_files" | grep -Fxq 'Arsenic/SettingsViewController.m'; then
        add_bullet "Track DarkSword toggle apply results independently in Settings" "settings"
    fi
    if printf '%s\n' "$changed_files" | grep -Fxq 'Arsenic/LogTextView.m'; then
        add_bullet "Tighten Log tab typography for dense verbose traces" "log view"
    fi
    if printf '%s\n' "$changed_files" | grep -Fxq 'Arsenic/tweaks/darksword_tweaks.m'; then
...
# 1b. Regenerate Arsenic/Changelog.plist with the new version as the top entry,
...
#     CURRENT_PROJECT_VERSION. build.sh writes build/Arsenic-${VERSION}.ipa and
#     refreshes a build/Arsenic.ipa symlink. We build *before* committing so the
...
APP_PATH="$PWD/build/DerivedData/Build/Products/Debug-iphoneos/Arsenic.app"
...
IPA="$PWD/build/Arsenic-${VERSION}.ipa"
if [ ! -f "$IPA" ]; then
...
if [ -f "$SOURCE_JSON" ]; then
    ...
    REPO_SLUG_FOR_JSON=$(echo "$ORIGIN_URL_FOR_JSON" \
        | sed -E 's#^(https?://[^/]+/|git@[^:]+:)##' \
        | sed -E 's#\.git$##')
    DOWNLOAD_URL="https://github.com/${REPO_SLUG_FOR_JSON}/releases/download/${EFFECTIVE_TAG}/Arsenic-${VERSION}.ipa"
