...
static uint64_t gStripPickerPendingBidLabel = 0;// Hidden label set by each tile tap; Arsenic reads on poll.
...
// Picker command codes carried in -[gStripPickerPanel tag]. Arsenic polls the
...
// ---------------------------------------------------------------------------
// Install the "arsenicStageStripOpen:" method on NSString. Its IMP points at
// SBSLaunchApplicationWithIdentifier, so when UIKit dispatches the gesture's
// action via objc_msgSend(bundleIdString, arsenicStageStripOpen:, sender) the
...
static bool stagestrip_ensure_open_method(void)
{
    ...
    uint64_t sel = r_sel("arsenicStageStripOpen:");
    if (!sel) {
    ...
    printf("[STAGE] open: arsenicStageStripOpen: installed on NSString added=%llu\n",
           added & 0xff);
    return true;
}
...
// app comes up in the foreground (used by the picker cog to open Arsenic's
// own settings app).
...
static bool stagestrip_assert_live_rendering_for_scene(uint64_t scene, const char *bid)
{
    ...
    char reason[192] = {0};
    snprintf(reason, sizeof(reason), "Arsenic StageStrip live render%s%s",
             (bid && *bid) ? " " : "", (bid && *bid) ? bid : "");
    ...
    uint64_t proxyKey = r_sel("arsenicStageStripLiveRenderingProxy");
    if (proxyKey) {
        r_dlsym_call(R_TIMEOUT, "objc_setAssociatedObject",
    ...
    if (!r_is_objc_ptr(bls) && r_is_objc_ptr(manager)) {
        ...
        if (r_is_objc_ptr(assertionProvider) && r_is_objc_ptr(attrs) &&
            ...
            uint64_t manualReason = r_nsstr_retained("Arsenic StageStrip direct BLS live render");
            if (r_is_objc_ptr(manualReason)) {
                ...
                if (r_is_objc_ptr(bls)) {
                    uint64_t blsKey = r_sel("arsenicStageStripBLSLiveRenderingAssertion");
                    if (blsKey) {
                        r_dlsym_call(R_TIMEOUT, "objc_setAssociatedObject",
    ...
    if (r_is_objc_ptr(bls) && r_responds(bls, "restartTimeoutTimer")) {
        ...
        uint64_t timerKey = r_sel("arsenicStageStripLiveRenderingTimer");
        if (timerKey) {
...
static uint64_t stagestrip_new_uuid_string(void)
{
    ...
    if (r_is_objc_ptr(string))
        return string;
    return r_nsstr_retained("ArsenicStageStripScene");
}
                                                            ...
                                                            const char *bid)
{
    ...
    uint64_t updaterKey = r_sel("arsenicStageStripSceneSettingsUpdater");
                                                         ...
                                                         const char *bid)
{
    ...
    uint64_t timerKey = r_sel("arsenicStageStripForegroundKeepaliveTimer");
                                                       ...
                                                       const char *bid)
{
    ...
    uint64_t timerKey = r_sel("arsenicStageStripExternalForegroundTimer");
    uint64_t timer = 0;
                                                ...
                                                const char *bid)
{
    ...
    uint64_t assertionKey = r_sel("arsenicStageStripRelevancyAssertions");
    ...
    if (!r_is_objc_ptr(assertions) && assertionKey) {
        ...
        uint64_t reason = r_nsstr_retained("Arsenic StageStrip live window");
...
static void stagestrip_invalidate_live_rendering_for_scene(uint64_t scene)
{
    ...
    uint64_t foregroundTimerKey = r_sel("arsenicStageStripForegroundKeepaliveTimer");
    ...
    uint64_t externalTimerKey = r_sel("arsenicStageStripExternalForegroundTimer");
    if (externalTimerKey) {
        uint64_t timer = r_dlsym_call(R_TIMEOUT, "objc_getAssociatedObject",
...
// Pick the first non-Arsenic application scene from [mgr allScenes] and
...
// that isn't Arsenic itself, resolve its persistence identifier to a scene
...
// which only lists what's currently "laid out" (i.e. just Arsenic when we
// run from the Arsenic app).
                                                      ...
                                                      double h)
{
    ...
    // is invalidated.
    uint64_t hostKey = r_sel("arsenicStageStripSceneLayerHost");
    ...
    uint64_t host = 0;
    uint64_t desc = r_nsstr_retained("Arsenic StageStrip scene host");
    ...
    uint64_t sceneKey = r_sel("arsenicStageStripHostedScene");
    if (sceneKey) {
        r_dlsym_call(R_TIMEOUT, "objc_setAssociatedObject",
                                                             ...
                                                             double w, double h)
{
    ...
    uint64_t overlayKey = r_sel("arsenicStageStripLiveContentOverlay");
    if (overlayKey) {
        r_dlsym_call(R_TIMEOUT, "objc_setAssociatedObject",
                                                  ...
                                                  double h)
{
    ...
    uint64_t vcKey = r_sel("arsenicStageStripMedusaViewController");
...
static uint64_t stagestrip_handle_make_view(uint64_t handle, double w, double h)
{
    ...
    // on the handle.
    uint64_t handleVCKey = r_sel("arsenicStageStripHandleViewController");
                                                   ...
                                                   double w, double h)
{
    ...
    for (int i = 0; i < count; i++) {
        ...
        uint64_t rowKey = r_sel(i == 0 ? "arsenicStageStripRow0" : "arsenicStageStripRow1");
        uint64_t liveKey = r_sel(i == 0 ? "arsenicStageStripLive0" : "arsenicStageStripLive1");
        if (rowKey) {
            r_dlsym_call(R_TIMEOUT, "objc_setAssociatedObject",
...
static void stagestrip_cleanup_host_view_depth(uint64_t view, int depth)
{
    ...
    const char *keys[] = {
        "arsenicStageStripMedusaViewController",
        "arsenicStageStripLiveContentOverlay",
        NULL,
    };
...
static void stagestrip_retain_action_target(uint64_t owner, uint64_t target)
{
    ...
    uint64_t key = r_sel("arsenicStageStripActionTargets");
    if (!key) return;
...
static uint64_t stagestrip_transition_shield_window(void)
{
    ...
    uint64_t assocKey = r_sel("arsenicStageStripTransitionShieldWindow");
...
static void stagestrip_relayout_stage_host_thread(uint64_t hostView, double w, double h)
{
    ...
    if (!r_is_objc_ptr(row0)) {
        uint64_t row0Key = r_sel("arsenicStageStripRow0");
        uint64_t live0Key = r_sel("arsenicStageStripLive0");
    ...
    if (!r_is_objc_ptr(row1)) {
        uint64_t row1Key = r_sel("arsenicStageStripRow1");
        uint64_t live1Key = r_sel("arsenicStageStripLive1");
...
// Arsenic on Apply), one updates the visible chip text+icon so the user can
// see their pick. The buttons never reach into Arsenic directly — Apply does.
...
// tag to kStripPickerCmdIconTap; Arsenic reads both on poll.
                                               ...
                                               uint64_t pendingBidLabel)
{
    ...
    // Tap → pending bid then tag. Arsenic's poll consumes both.
    uint64_t bidStr = r_nsstr_retained(bid);
    if (r_is_objc_ptr(bidStr)) {
...
// within a single Arsenic session, so we cache it after the first build and
...
// inside SpringBoard via NSInvocations; Arsenic only ever reads
                                                  ...
                                                  double sh)
{
    ...
    uint64_t cacheKey = r_sel("arsenicStageStripPickerWindow");
    ...
    //     the "next slot" pointer in Arsenic so the user can control which
    ...
    for (int slot = 0; slot < 2; slot++) {
        ...
        // Tap gesture on the card → set the "next slot" pointer in Arsenic.
        ...
        // Icon UIImageView (tile taps -setImage: into this via Arsenic).
        uint64_t UIImageView = r_class("UIImageView");
    ...
    //  kStripPickerCmdClose before Arsenic could poll. Users dismiss via the
    //  explicit "x" button.)
                                                 ...
                                                 double sh)
{
    ...
    uint64_t assocKey = r_sel("arsenicStageStripHotCornerWindow");
    if (!assocKey) return;
                                                     ...
                                                     double defaultY)
{
    ...
    char keyName[64];
    snprintf(keyName, sizeof(keyName), "arsenicStageStripFloatWindow%d", slot);
...
static bool stagestrip_dismiss_floating_host(void)
{
    ...
    // (arsenicStageStripFloatWindowN).
    for (int s = 0; s < kStripMaxFloatSlots; s++) {
        char keyName[64];
        snprintf(keyName, sizeof(keyName), "arsenicStageStripFloatWindow%d", s);
    ...
    uint64_t legacyKey = r_sel("arsenicStageStripFloatWindow");
    ...
    uint64_t hotKey = r_sel("arsenicStageStripHotCornerWindow");
    ...
    uint64_t pickerKey = r_sel("arsenicStageStripPickerWindow");
    ...
    uint64_t shieldKey = r_sel("arsenicStageStripTransitionShieldWindow");
...
// non-Arsenic handle from the App-Switcher recents (which survive even
// when Arsenic is foregrounded), and falls back to layoutStateApplicationSceneHandles
...
static bool stagestrip_install_or_refresh(int maxSlots)
{
    ...
    uint64_t assocKey = r_sel("arsenicStageStripOverlayWindow");
    if (!assocKey) return false;
...
static int stagestrip_poll_picker_command(void)
{
    ...
    switch (cmd) {
        ...
        case kStripPickerCmdRespring: {
            // Gear icon on the picker → open the Arsenic settings app. (We
            ...
            // connection mid-call. Arsenic's settings UI has its own
            // respring button that uses the WebKit payload — the user can
            // hit that one after we foreground the app.)
            printf("[STAGE] picker: cog tapped — launching Arsenic app\n");
            stagestrip_hide_picker_overlay_animated();
            stagestrip_launch_foreground("com.iggy05.ios-arsenic");
            return cmd;
        }
