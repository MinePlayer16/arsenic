...
static NSString *nbl_date_with_format(NSString *format)
{
    ...
    if ([format isEqualToString:@"arsenic:lunar"]) return nbl_lunar_date_text();
    if ([format isEqualToString:@"arsenic:lunar-cn"]) return nbl_lunar_date_text_cn(false);
    if ([format isEqualToString:@"arsenic:lunar-cn-full"]) return nbl_lunar_date_text_cn(true);
    if ([format isEqualToString:@"arsenic:cn-date-weekday"] || [format isEqualToString:@"M月d日 EEE"]) {
...
static bool nbl_create_or_fetch_window(void)
{
    ...
    uint64_t assocKey = r_sel("arsenicNiceBarLiteWindow");
