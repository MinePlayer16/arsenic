...
- (void)buildTweakDocsInStack:(UIStackView *)stack
{
    [stack addArrangedSubview:[self introCard:@"Build JavaScript Tweaks"
                                        body:@"Write .js scripts that run inside SpringBoard through Arsenic's RemoteCall bridge. Publish them as a source repo or load them locally with QuickLoader."
    ...
    [stack addArrangedSubview:[self docCard:@"Writing a Tweak"
        body:@"A tweak is a plain .js file that runs inside JavaScriptCore. 64-bit pointers are passed as hex strings (e.g. \"0x105f2a000\"). Wrap your code in an IIFE:\n\n"
              ...
              "  // …\n"
              "})();\n\n"
              "Use log() to print to Arsenic's Log tab. Use r_msg2_main() instead of r_msg2() for anything that touches UIKit views."]];

    [stack addArrangedSubview:[self docCard:@"Parameter Declaration"
        body:@"Add @param comments at the top of your script. Arsenic parses them and generates native UI controls in Settings.\n\n"
              "Syntax:\n"
    ...
    [stack addArrangedSubview:[self docCard:@"RemoteCall API"
        body:@"These functions are injected into every JS context:\n\n"
              "log(message)\n"
              "  Print to Arsenic's log console.\n\n"
    ...
    [stack addArrangedSubview:[self docCard:@"Advanced API & Memory"
        body:@"r_is_objc_ptr(ptr) → bool\n"
              "  Validate if a pointer is a valid ObjC object.\n\n"
              "r_alloc_str(str) / r_free(ptr)\n"
              "  Allocate or free a C-string in the target.\n\n"
              "r_cfstr(str) → pointer\n"
              "  Allocate a CFString (must CFRelease it).\n\n"
              "r_ivar_value(target, \"_name\") → pointer\n"
              "  Read an instance variable directly.\n\n"
              "r_dlsym_call(timeout, \"sym\", a1...a8)\n"
              "  Call any C function by symbol name.\n\n"
              "r_msg2_main_struct_ret_rect(target, sel)\n"
              "  Returns a CGRect as a JS object: {x, y, w, h}.\n\n"
              "r_msg2_main_raw_double(t, sel, d1, d2, d3)\n"
              "  Pass up to 3 doubles by value to a method.\n\n"
              "remote_read(addr, size) → array\n"
              "remote_write(addr, array)\n"
              "  Read/write raw memory using JS byte arrays."]];
    ...
    [stack addArrangedSubview:[self docCard:@"Timers"
        body:@"Standard timer functions are mapped to GCD background queues so they never block the UI:\n\n"
              ...
              "setTimeout(fn, ms) → id\n"
              "clearTimeout(id)\n\n"
              "All active timers are automatically cancelled when the tweak is disabled or Arsenic cleans up."]];
...
- (void)buildRepoDocsInStack:(UIStackView *)stack
{
    [stack addArrangedSubview:[self introCard:@"Set Up a Tweak Repository"
                                        body:@"Host your JavaScript tweaks as an HTTPS JSON feed that Arsenic users can add in the Sources tab. No server-side code needed — a GitHub Pages repo or any static file host works."
                                      credit:nil]];
    ...
    [stack addArrangedSubview:[self docCard:@"Repository JSON Format"
        body:@"Create a JSON file with this structure:\n\n"
              ...
              "  ]\n"
              "}\n\n"
              "Host it at any HTTPS URL. Users paste this URL into Arsenic's Sources tab."]];
    ...
    [stack addArrangedSubview:[self docCard:@"Required Fields"
        body:@"repoName\n"
              ...
              "tweaks[].version\n"
              "  Semver string. Arsenic compares this to\n"
              "  detect updates for installed tweaks.\n\n"
              "tweaks[].scriptURL\n"
              "  HTTPS URL to the .js file. Arsenic\n"
    ...
    [stack addArrangedSubview:[self docCard:@"Updating Tweaks"
        body:@"To push an update:\n\n"
              ...
              "3. Commit and push\n\n"
              "Arsenic checks for updates every few hours. When the repo version is newer than the installed version, the Sources tab shows an update badge. Users tap the tweak to reinstall the latest version."]];
    ...
    [stack addArrangedSubview:[self docCard:@"Tips"
        body:@"• Use descriptive ids like \"yourname.tweakname\" to avoid collisions with other repos.\n"
              ...
              "• HTTPS is required — Arsenic rejects HTTP URLs."]];
}
...
- (void)shareMarkdown
{
    ...
    NSString *name = (self.docsMode == JSTweakDocsModeSetupRepo) ? @"Arsenic-Repo-Setup.md" : @"Arsenic-JS-Tweak-Docs.md";
...
{
    return
    @"# Arsenic JavaScript Tweak Documentation\n\n"
    ...
    "| `r_is_objc_ptr(p)` | Validate ObjC pointer |\n"
    "| `r_alloc_str(s)` / `r_free(p)` | C-string alloc/free |\n"
    "| `r_cfstr(s)` | CFString alloc (needs release) |\n"
    "| `r_ivar_value(t, name)` | Read instance variable |\n"
    "| `r_dlsym_call(t, s, a1..a8)` | Call C function |\n"
    "| `r_msg2_main_struct_ret_rect(t,s)`| Returns JS Object `{x,y,w,h}` |\n"
    "| `r_msg2_main_raw_double(t,s,d1..)`| Pass doubles by value |\n"
    "| `remote_read`/`write` | Raw memory access via arrays |\n\n"
    "## QuickLoader Prefs\n\n"
...
- (NSString *)repoMarkdown
{
    ...
    "3. Push — Arsenic detects updates automatically\n";
}
