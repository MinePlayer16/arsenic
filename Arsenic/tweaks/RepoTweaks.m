...
static dispatch_queue_t repotweaks_create_js_queue_locked(void) {
    NSString *label = [NSString stringWithFormat:@"com.iggy05.arsenic.repotweaks.js.%llu",
...
static NSString *repotweaks_cache_busted_url_string(NSString *urlString) {
    ...
    for (NSInteger i = (NSInteger)items.count - 1; i >= 0; i--) {
        if ([items[(NSUInteger)i].name isEqualToString:@"_arsenicBust"]) {
    ...
    [items addObject:[NSURLQueryItem queryItemWithName:@"_arsenicBust" value:stamp]];
