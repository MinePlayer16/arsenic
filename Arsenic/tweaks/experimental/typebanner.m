...
// can wedge runningboardd's RBAssertionManager while Arsenic is blocked in the
...
static uint64_t tb_find_existing_window(void)
{
    ...
    uint64_t assocKey = r_sel("arsenicTypeBannerOverlayWindow");
...
{
    return r_sel("arsenicTypeBannerMobileSMSKeepAliveAssertion");
}
...
bool typebanner_ensure_mobilesms_keepalive_in_springboard_session(uint32_t pid)
{
    ...
    uint64_t attributes = r_is_objc_ptr(legacy)
        ...
        : 0;
    NSString *nameString = [NSString stringWithFormat:@"Arsenic TypeBanner MobileSMS pid %u", pid];
